...
namespace chatllm
{
    ...
    std::vector<int> BaseTokenizer::encode_embedding(const std::string& text, EmbeddingPurpose purpose) const
    {
        std::vector<int> ids;
        encode(text, ids);
        return ids;
    ...
    std::vector<int> BaseTokenizer::encode_embedding(const Content& input, EmbeddingPurpose purpose) const
    {
        CHATLLM_CHECK(input.is_simple_text());
        return encode_embedding(input.to_string(), purpose);
    }
        ...
        ggml::type target_type, ggml::tensor* src_tensor)
    {
        ...
        if (target_type != original_type)
        {
            ...
            else
            {
                ...
                ggml::tensor t;
                ggml::init_tensor(&t, ggml::type::GGML_TYPE_F32, 4, src_tensor->ne.data());
                ...
                {
                    ggml::init_tensor(&t, original_type, 4, src_tensor->ne.data());
                ...
                ggml::init_tensor(&t, target_type, 4, src_tensor->ne.data());
                buf_q.resize(ggml::nbytes(&t));
...
#define GGML_QK8_0 32
    ...
    size_t TensorInfo::read_tensor_data_f32_f16(tokenizer::DataReader* reader, size_t read_offset, size_t write_offset, size_t data_size)
    {
        ...
        {
            p16[i] = fromFloat32<ggml_fp16_t>(p32[i]);
        }
    ...
    {
        alloc_managers.emplace_back(alloc_manager);
    }

    void ModelLoader::pop_allocator_manager()
    {
    ...
    {
        read_tensor(name, tensor, this->alloc_manager().get_allocator(tensor), partial);
    }
        ...
        ggml::tensor* tensor, LayerBufAllocator* allocator)
    {
        ...
        tensor_dict.emplace(name, TensorInfo(tensor->type, 4, tensor->ne.data(), 0, name.c_str()));
    ...
    BaseTokenizer& Pipeline::tokenizer() {
        return *modelobj.tokenizer;
    }
        ...
        BaseStreamer* streamer)
    {
        ...
        std::string output = tokenizer().decode(result.ids);
        return output;
    }
    ...
    std::vector<float> Pipeline::embedding(const Content& input, const GenerationConfig& gen_config, BaseTokenizer::EmbeddingPurpose purpose)
    {
        if (!modelobj.loaded) return {};
        std::vector<int> input_ids = tokenizer().encode_embedding(input, purpose);
        return model().embedding(gen_config, input_ids);
    }
    ...
    std::string RAGPipeline::rewrite_query(const std::string& prompt, const GenerationConfig& gen_config)
    {
        ...
        AbstractModel::generate_result result = rewrite_model->generate(input_ids, gen_config, false, &performance, nullptr);

        if (!result.completed) return "";

        return tokenizer().decode(result.ids);
    }
    ...
    void RAGPipeline::before_chat(Messages& history, const GenerationConfig& gen_config, BaseStreamer* streamer)
    {
        ...
        std::vector<float> query_emb = embedding(Content(nullptr, rewritten_query), gen_config, BaseTokenizer::EmbeddingPurpose::Query);
    ...
    static void log_soft_max(std::span<float> vector)
    {
        float max_score = *std::max_element(vector.begin(), vector.end());
        float sum = 0.f;
        for (const float& s : vector)
        {
    ...
    static std::vector<int> topk_sampling(std::span<const float> logits, int top_k)
    {
        struct TokenIdScore
        {
            int id;
    ...
    void BeamSearchPipeline::do_chat0(Messages& history, const GenerationConfig& gen_config, BaseStreamer* streamer)
    {
        ...
        std::vector<int> selected_ids = topk_sampling(lm_logits, (int)beams.size());
    ...
    void BeamSearchPipeline::do_chat(Messages& history, const GenerationConfig& gen_config, BaseStreamer* streamer)
    {
        ...
        while (!completed)
        {
            ...
            std::vector<int> selected_ids = topk_sampling(all_logits, (int)beams.size());
