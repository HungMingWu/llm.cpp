...
namespace chatllm::qwen::v3::audio_tower
{
    ...
    AudioEmbedding::AudioEmbedding(InitContext* ctx, const Config& config) :
        ...
        positional_embedding(ctx->new_tensor(ggml::type::GGML_TYPE_F32, { config.max_source_positions, config.hidden_size }))
    {
    ...
    ggml::tensor* AudioEmbedding::forward(ComputeContext* ctx, ggml::tensor* input)
    {
        ...
        input = ctx->reshape(input, {hidden_size, ggml::get_dim(input, 0) / pad_window, pad_window});
        ...
        hidden_states = ctx->reshape(hidden_states, { ggml::get_dim(hidden_states, 3), ggml::get_dim(hidden_states, 2), ggml::get_dim(hidden_states, 0) * ggml::get_dim(hidden_states, 1)});
        ...
        const BaseTokenizer::MediaAsEmbeddingVector& audio, std::vector<uint8_t>& buf)
    {
        ...
        const auto make_graph = [this, &media_emb, &audio, n_len](ComputeContext* ctx) -> ggml::tensor* {
            media_emb = ctx->new_tensor(ggml::type::GGML_TYPE_F32, { config.feature_size, n_len });;
