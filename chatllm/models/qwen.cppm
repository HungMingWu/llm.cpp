...
namespace chatllm::qwen
{
    ...
    namespace vit
    {
        ...
        public:
            ...
            {
                Base::mask = full_attention ? nullptr : ctx->new_tensor(GGML_TYPE_F32, { max_length, max_length });
        ...
        protected:
            ggml::tensor* apply_2d_rope(ComputeContext* ctx, ggml::tensor* hidden, int hidden_size) const
            {
                ...
                hidden = ggml::rope_ext(ctx, hidden, Base::pos, Base::freq_factors, Base::rope_dim / 2, GGML_ROPE_TYPE_VISION, Base::n_original_ctx,
                ...
                ggml::tensor* attn_scores) override
            {
                ...
                ggml::tensor* sub_mask = Base::mask ? ctx->view(Base::mask, { qlen, qlen },
