...
namespace chatllm::qwen::vit
{
    ...
    bool VisualEmbeddingGeneration::run_model(const GenerationConfig& gen_config, BaseTokenizer* tok, ggml::type dtype, const BaseTokenizer::MediaAsEmbeddingVector& image, std::vector<uint8_t>& buf)
    {
        ...
        ctx.move_to_layer(LayerAllocatorManager::MiscLayer::Prolog);
        ggml::tensor* image1 = ctx.new_tensor(ggml::type::GGML_TYPE_F32,
            { image.grid_width * image.grid_height, 3, vis_config.patch_size, vis_config.patch_size });
        ggml::tensor* image2 = ctx.new_tensor(ggml::type::GGML_TYPE_F32,
            { image.grid_width * image.grid_height, 3, vis_config.patch_size, vis_config.patch_size });
...
namespace chatllm::qwen::v3_vl::vit
{
    ...
    bool VisualEmbeddingGeneration::run_model(const GenerationConfig& gen_config, BaseTokenizer* tok, ggml::type dtype, const BaseTokenizer::MediaAsEmbeddingVector& image, std::vector<uint8_t>& buf)
    {
        ...
        const auto make_graph = [this, &image1, &image2, &image](ComputeContext* ctx) -> ggml::tensor* {
            image1 = ctx->new_tensor(ggml::type::GGML_TYPE_F32, { image.grid_width * image.grid_height, 3, vis_config.patch_size, vis_config.patch_size });
            image2 = ctx->new_tensor(ggml::type::GGML_TYPE_F32, { image.grid_width * image.grid_height, 3, vis_config.patch_size, vis_config.patch_size });
            auto r = vis_model->forward(ctx, image1, image2, image.grid_height, image.grid_width);
            return r;
        };
...
namespace chatllm::qwen::v3_vl
{
    ...
    ggml::tensor* ConditionalGeneration::lm_layer_preprocess(HeterogeneousModel* model, ComputeContext* ctx, ggml::tensor* hidden_states, int layer_index)
    {
        ...
        auto r = ggml::add(ctx, hidden_states, emb, false);
        return r;
    }
}
...
namespace chatllm::qwen::v3_forcedaligner
{
    ...
    void fix_timestamp(const std::vector<double>& data, std::vector<double>& timestamps)
    {
     ...
     AbstractModel::generate_result ConditionalGeneration::generate(std::span<const int> input_ids, const GenerationConfig& gen_config,
        const bool continuous,
        ModelPerfInfo* performance,
        BaseStreamer* streamer)
    {
        ...
        run_model(input_ids, gen_config, n_past, lm_ordering, 1,
            sort_logits);
        ...
        return { std::move(output_ids), true };
    }
