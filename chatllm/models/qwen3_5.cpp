...
namespace chatllm::qwen::v3_5
{
    ...
    public:
        ...
        void before_run_model(std::span<const int> input_ids, const GenerationConfig& gen_config, int past) override;
    ...
    CausalConv1D::CausalConv1D(InitContext* ctx, int in_channels, int out_channels, int kernel_size, int stride,
        ...
        state(ctx->new_tensor(ggml::type::GGML_TYPE_F32, { in_channels, kernel_size - 1 }))
    {
    ...
    ggml::tensor* CausalConv1D::forward(ComputeContext* ctx, ggml::tensor* input, int n_past)
    {
        ...
        if (n_past < 1)
        {
            auto clear = ggml::scale(ctx, state, 0.0f, false);
        ...
        input = ggml::concat(ctx, state, input, 0);
        {
            ...
            auto hist = ctx->view(input, { ggml::get_dim(input, 2), ggml::get_dim(input, 1), hist_size },
    ...
    ggml::tensor* RMSNormGated::forward(ComputeContext* ctx, ggml::tensor* input, ggml::tensor* gate)
    {
        ...
        hidden_states = ggml::mul(ctx, hidden_states, w, false);
        ...
        ggml::tensor* query_layer, ggml::tensor* key_layer, ggml::tensor* v)
    {
        ...
        attn_scores = ggml::mul(ctx, attn_scores, g, false);
        return attn_scores;
    }
    ...
    QwenGatedDeltaNet::QwenGatedDeltaNet(InitContext* ctx, int hidden_size, int conv_kernel_dim, int num_key_heads, int num_value_heads, int key_head_dim, int value_head_dim) :
        ...
        state(ctx->new_tensor(ggml::type::GGML_TYPE_F16, { num_v_heads, head_k_dim, head_v_dim })),
        conv1d(ctx, key_dim * 2 + value_dim, key_dim * 2 + value_dim, conv_kernel_dim, 1, 1, key_dim * 2 + value_dim, false),
        dt_bias(ctx->new_tensor(ggml::type::GGML_TYPE_F32, { num_value_heads })),
        A_log(ctx->new_tensor(ggml::type::GGML_TYPE_F32, { num_value_heads })),
    ...
    ggml::tensor* QwenGatedDeltaNet::calc_g(ComputeContext* ctx, ggml::tensor* input)
    {
        ...
        auto g = ggml::add(ctx, a, dt_bias, false);
        g = ggml::softplus(ctx, g);
        g = ggml::mul(ctx, g, ggml::exp(ctx, A_log), false);
        g = ggml::neg(ctx, g);
    ...
    {
        CHATLLM_CHECK(ggml_n_dims(key) <= 3);
        ...
        auto get_t = [](ComputeContext* ctx, ggml::tensor* x, int t) {
            auto r = ctx->view(x, { ggml::get_dim(x, 2), ggml::get_dim(x, 0) },
        ...
        for (int i = 0; i < sequence_length; i++)
        {
            ...
            last_state = ggml::mul(ctx, last_state, g_t, false);
            auto kv_mem = ggml::mul(ctx, last_state, k_t, false);
            kv_mem = ggml::sum(ctx, kv_mem, 1);
            auto delta = ggml::sub(ctx, v_t, kv_mem, false);
            delta = ggml::mul(ctx, delta, beta_t, false);
            ...
            delta = ggml::mul(ctx, k_t, delta, false);
            last_state = ggml::add(ctx, last_state, delta, false);

            auto attn_t = ggml::mul(ctx, last_state, q_t, false);
    ...
    ggml::tensor* QwenGatedDeltaNet::forward(ComputeContext* ctx, ggml::tensor* input, int n_past)
    {
        ...
        auto query = ctx->view(mixed_qkv, 
            { ggml::get_dim(mixed_qkv, 3), ggml::get_dim(mixed_qkv, 2), ggml::get_dim(mixed_qkv, 1), key_dim },
            ...
            0);
        auto key = ctx->view(mixed_qkv,
            { ggml::get_dim(mixed_qkv, 3), ggml::get_dim(mixed_qkv, 2), ggml::get_dim(mixed_qkv, 1), key_dim },
            { ggml::row_size(mixed_qkv) * ggml::get_dim(mixed_qkv, 1) * ggml::get_dim(mixed_qkv, 2),
              ggml::row_size(mixed_qkv) * ggml::get_dim(mixed_qkv, 1),
              ggml::row_size(mixed_qkv) },
            ggml::element_size(mixed_qkv) * key_dim);
        auto value = ctx->view(mixed_qkv,
            { ggml::get_dim(mixed_qkv, 3), ggml::get_dim(mixed_qkv, 2), ggml::get_dim(mixed_qkv, 1), key_dim },
            { ggml::row_size(mixed_qkv) * ggml::get_dim(mixed_qkv, 1) * ggml::get_dim(mixed_qkv, 2),
              ggml::row_size(mixed_qkv) * ggml::get_dim(mixed_qkv, 1),
              ggml::row_size(mixed_qkv) },
        ...
        core_attn_out = ctx->reshape(core_attn_out, { ggml::nelements(core_attn_out) / head_v_dim, head_v_dim });
        ...
        visual(runtime_config, pad_arg->get())
    {
        ...
        transformer = new ModelClass(&w_ctx_, config.num_hidden_layers, config.hidden_size,
            ...
            config.tie_word_embeddings ? nullptr : create_lm_head(&w_ctx_, config, false),
    ...
    void ConditionalGeneration::before_run_model(std::span<const int> input_ids, const GenerationConfig& gen_config, int past)
    {
