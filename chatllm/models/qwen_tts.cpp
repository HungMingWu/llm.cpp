...
namespace chatllm::qwen::tts
{
    ...
    ggml::tensor* ResidualVectorQuantization::forward(ComputeContext* ctx, ggml::tensor* input)
    {
        ...
        for (int i = 0; i < num_quantizers; i++)
        {
            auto code = ctx->view(input, { ggml::get_dim(input, 1), ggml::get_dim(input, 0) },
                { ggml::row_size(input) },
                ggml::row_size(input) * ggml::get_dim(input, 1) * i);
            auto r = layers.blocks[i]->forward(ctx, code);
            quantized = quantized ? ggml::add(ctx, quantized, r, false) : r;
        }
        return quantized;
    }
    ...
    {
        auto part = ctx->view(input, { ggml::get_dim(input, 2), n_q_semantic, ggml::get_dim(input, 0) },
            { ggml::row_size(input) * ggml::get_dim(input, 1), ggml::row_size(input) }, 0);
        auto quantized = rvq_first->forward(ctx, part);
        if (ggml::get_dim(input, 1) > n_q_semantic)
        {
            part = ctx->view(input, { ggml::get_dim(input, 2), ggml::get_dim(input, 1) - n_q_semantic, ggml::get_dim(input, 0) },
            ...
            quantized = ggml::add(ctx, quantized, rest, false);
        }
        return quantized;
    }
    ...
    ggml::tensor* Qwen3TTSTokenizerV2CausalTransConvNet::forward(ComputeContext* ctx, ggml::tensor* input)
    {
        ...
        hidden_state = ctx->view(hidden_state,
            { ggml::get_dim(hidden_state, 3), ggml::get_dim(hidden_state, 2), ggml::get_dim(hidden_state, 1), ggml::get_dim(hidden_state, 0) - left_pad - right_pad },
            { ggml::row_size(hidden_state) * ggml::get_dim(hidden_state, 1) * ggml::get_dim(hidden_state, 2),             ggml::row_size(hidden_state) * ggml::get_dim(hidden_state, 1),             ggml::row_size(hidden_state) },
    ...
    Qwen3TTSTokenizerV2ConvNeXtBlock::Qwen3TTSTokenizerV2ConvNeXtBlock(InitContext* ctx, int dim) :
        ...
        gamma(ctx->new_tensor(ggml::type::GGML_TYPE_F32, { dim }))
    {
    }
    ...
    ggml::tensor* Qwen3TTSTokenizerV2ConvNeXtBlock::forward(ComputeContext* ctx, ggml::tensor* input)
    {
        ...
        hidden_states = ggml::mul(ctx, hidden_states, gamma, false);

        hidden_states = ggml::permute(ctx, hidden_states, 1, 0, 2);

        hidden_states = ggml::add(ctx, hidden_states, input, false);
    ...
    SnakeBeta::SnakeBeta(InitContext* ctx, int dim) :
        ...
        alpha(ctx->new_tensor(ggml::type::GGML_TYPE_F32, { dim })),
        beta(ctx->new_tensor(ggml::type::GGML_TYPE_F32, { dim }))
    {
    }

    ggml::tensor* SnakeBeta::forward(ComputeContext* ctx, ggml::tensor* input)
    {
        auto alpha = ctx->reshape(this->alpha, { 1, ggml::get_dim(this->alpha, 0), 1 });
        auto beta = ctx->reshape(this->beta, { 1, ggml::get_dim(this->beta, 0), 1 });

        alpha = ggml::mul(ctx, input, alpha, false);
        alpha = ctx->sin(alpha);
        alpha = ctx->square(alpha);
        alpha = ggml::mul(ctx, alpha, beta, false);

        auto hidden_states = ggml::add(ctx, input, alpha, false);
        return hidden_states;
    }
    ...
    ggml::tensor* Qwen3TTSTokenizerV2DecoderAttention::forward(ComputeContext* ctx, ggml::tensor* input, int n_past)
    {
        ...
        {
            hidden_states = ggml::mul(ctx, hidden_states, self_attn_layer_scale, false);
        }
        return hidden_states;
    }
    ...
    ScaledSiLUMLP::ScaledSiLUMLP(InitContext* ctx, int hidden_size, int intermediate_size) :
        ...
        scale(ctx->new_tensor(ggml::type::GGML_TYPE_F32, { hidden_size }))
    {
    }
    ...
    ggml::tensor* ScaledSiLUMLP::forward(ComputeContext* ctx, ggml::tensor* hidden_states)
    {
        ...
        hidden_states = ggml::mul(ctx, hidden_states, scale, false);
        return hidden_states;
    }
    ...
    Qwen3TTSTokenizerV2DecoderTransformerLayer::Qwen3TTSTokenizerV2DecoderTransformerLayer(InitContext* ctx,
        ...
        self_attn_layer_scale(ctx->new_tensor(ggml::type::GGML_TYPE_F32, { config.hidden_size }))
    {
    ...
    ggml::tensor* Qwen3TTSTokenizerV2DecoderTransformerModel::forward(ComputeContext* ctx, ggml::tensor* input)
    {
        ...
        ggml::tensor* sub_mask = ctx->view(mask, { qlen, qlen }, { max_length * ggml::element_size(mask) }, 0);
        ...
        const int chunk_size, const int left_context_size)
    {
        ...
        codes = ggml_is_contiguous(codes) ? codes : ggml::cont(ctx, codes);
        ...
        while (start_index < qlen)
        {
            ...
            auto chunk = ctx->view(codes,
                { (end_index - start_index + context_size), ggml::get_dim(codes, 0) },
            ...
            wav_chunk = ctx->view(wav_chunk, { sample_cnt - (context_size * total_upsample), ggml::get_dim(wav_chunk, 0)  },
    ...
    ggml::tensor* Qwen3TTSTalkerCodePredictorModelForConditionalGeneration::forward(ComputeContext* ctx, ggml::tensor* inputs_embeds, int n_past, int generation_step)
    {
        ...
        auto sub = ctx->view(hidden_states, { 1, ggml::get_dim(hidden_states, 0) }, { ggml::row_size(hidden_states) }, offset);
    ...
    ggml::tensor* Qwen3TTSTalkerCodePredictorModelForConditionalGeneration::forward_finetune(ComputeContext* ctx, ggml::tensor* inputs_embeds)
    {
        ...
        for (int i = 1; i < num_code_groups; i++)
        {
            ...
            auto sub = ctx->view(hidden_states, { 1, hidden_size }, { ggml::row_size(hidden_states) }, offset);
    ...
    ggml::tensor* Res2NetBlock::forward(ComputeContext* ctx, ggml::tensor* inputs)
    {
        ...
        {
            auto hidden_part = ctx->view(inputs,
                { ggml::get_dim(inputs, 3), ggml::get_dim(inputs, 2), dim / scale, ggml::get_dim(inputs, 0) },
                { ggml::row_size(inputs) * ggml::get_dim(inputs, 1) * ggml::get_dim(inputs, 2), ggml::row_size(inputs) * ggml::get_dim(inputs, 1),ggml::row_size(inputs)  },
            ...
            {
                output = ggml::add(ctx, output, hidden_part, false);
    ...
    ggml::tensor* SqueezeExcitationBlock::forward(ComputeContext* ctx, ggml::tensor* inputs)
    {
        ...
        hidden_states_mean = ggml::mul(ctx, hidden_states_mean, inputs, false);
    ...
    void AttentiveStatisticsPooling::_compute_statistics(ComputeContext* ctx, ggml::tensor* x, ggml::tensor*& mean, ggml::tensor*& std)
    {
        ...
        mean = ggml::scale(ctx, mean, 1.0f / total, false);

        std = ggml::sub(ctx, x, mean, false);
        std = ctx->square(std);
        std = ggml::sum_rows(ctx, std);
        std = ggml::scale(ctx, std, 1.0f / total, false);
        std = ctx->sqrt(std);
    }
    ...
    void AttentiveStatisticsPooling::_compute_statistics(ComputeContext* ctx, ggml::tensor* x, ggml::tensor* m, ggml::tensor*& mean, ggml::tensor*& std)
    {
        ...
        mean = ggml::mul(ctx, x, m, false);
        mean = ggml::sum_rows(ctx, mean);

        std = ggml::sub(ctx, x, mean, false);
        std = ctx->square(std);
        std = ggml::mul(ctx, std, m, false);
        std = ggml::sum_rows(ctx, std);
        std = ctx->sqrt(std);
    }
    ...
    ggml::tensor* AttentiveStatisticsPooling::forward(ComputeContext* ctx, ggml::tensor* inputs)
    {
        ...
        CHATLLM_CHECK(ggml_n_dims(inputs) <= 2);
        ...
        attention = ggml::soft_max(ctx, attention, false);
    ...
    ggml::tensor* SqueezeExcitationRes2NetBlock::forward(ComputeContext* ctx, ggml::tensor* inputs)
    {
        ...
        hidden_state = ggml::add(ctx, hidden_state, residual, false);
        return hidden_state;
    }
...
namespace chatllm::qwen::v3_tts
{
    ...
    int TalkerGeneration::code_predict(const GenerationConfig& gen_config, Sampler* sampler, const int* input_text_ids, const int id_num, int n_past, const int generation_step)
    {
        ...
        const auto make_graph = [this, &input, input_text_ids, id_num, n_past, generation_step](ComputeContext* ctx) -> ggml::tensor* {
            input = ctx->new_tensor(ggml::type::GGML_TYPE_I32, { id_num });
        ...
        int id = sampler->sampling(std::span{ (float*)buf.data(), (size_t)code_predictor->vocab_size });
        return id;
    }
    ...
    int TalkerGeneration::code_predict(const GenerationConfig& gen_config, Sampler* sampler, ggml::type dtype, const float* past_hidden, const float* last_id_hidden, int n_past, const int generation_step)
    {
        ...
        const auto make_graph = [this, &input, dtype, n_past, generation_step](ComputeContext* ctx) -> ggml::tensor* {
            input = ctx->new_tensor(dtype, { 2, embedding_dim });
            auto r = code_predictor->forward(ctx, input, n_past, generation_step);
            return r;
        };
    ...
    void TalkerGeneration::speech_decode(const GenerationConfig& gen_config, const std::vector<int>& encoded, std::vector<float>& decoded)
    {
        ...
        const auto make_graph = [this, &encoded, &input](ComputeContext* ctx) -> ggml::tensor* {
            input = ctx->new_tensor(ggml::type::GGML_TYPE_I32, { code_block_size, (int64_t)encoded.size() / code_block_size });
        ...
        ggml::type dtype, std::vector<uint8_t>& buf)
    {
        ...
        const auto make_graph = [this, &input, &mel](ComputeContext* ctx) -> ggml::tensor* {
            input = ctx->new_tensor(ggml::type::GGML_TYPE_F32, {mel.n_len, mel.n_mel});
            auto r = input;
            r = speaker_encoder->forward(ctx, r);
            r = ctx->view(r, { ggml::get_dim(r, 0) }, {}, 0);
            return r;
        };
    ...
    void ConditionalGeneration::load(ModelLoader& loader)
    {
        ...
        if (talker.loaded)
        {
            transformer->lm_head = std::move(talker.codec_head);
        }
    }
    ...
    int ConditionalGeneration::run_main_model(const GenerationConfig& gen_config, std::vector<float>& last_hidden_states, Sampler* sampler)
    {
        ...
        int id = sampler->sampling(std::span{ logits.data(), (size_t)talker_vocab_size });
        return id;
    }
    ...
    int ConditionalGeneration::run_main_model(const GenerationConfig& gen_config, const int added_id, const std::vector<int>& code_block, std::vector<float>& last_hidden_states, Sampler* sampler)
    {
        ...
        {
            input_ids.push_back(ctx.new_tensor(GGML_TYPE_I32, { 1 }));
        }
        ggml::tensor* input_id_new = ctx.new_tensor(GGML_TYPE_I32, { 1 });
        ...
        const auto custom_embedding = [this, input_ids, input_id_new, code_block](ComputeContext* ctx, ggml::tensor* input)
        {
            ...
            for (int i = 1; i < (int)code_block.size(); i++)
            {
                ...
                r = ggml::add(ctx, r, emb, false);
            }

            auto pad_emb = transformer->word_embeddings->forward(ctx, input_id_new);
            auto output = ggml::add(ctx, r, pad_emb, false);
            return output;
        };
        ...
        auto id = sampler->sampling(std::span{ logits.data(), (size_t)talker_vocab_size });
        return id;
    }
        ...
        std::vector<float>& output, std::vector<float>& last_hidden_states)
    {
        ...
        ggml::tensor* added_ids_tensor = ctx.new_tensor(GGML_TYPE_I32, { ids_count });
        ggml::tensor* added_scale = ctx.new_tensor(GGML_TYPE_F32, { ids_count, 1 });
        ...
        const auto custom_embedding = [this, added_ids_tensor, added_scale](ComputeContext* ctx, ggml::tensor* input)
        {
                ...
                added = ggml::mul(ctx, added, added_scale, false);
                output = ggml::add(ctx, output, added, false);
                return output;
        };
...
1581, 189 });
        ...
        std::vector<int16_t>& audio)
    {
        ...
        while ((gen_config.max_new_tokens <= 0) || (codec_ids.size() < (int)gen_config.max_new_tokens))
        {
            ...
            auto emb = dynamic_cast<Embedding*>(transformer->word_embeddings.get());
