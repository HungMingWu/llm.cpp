...
namespace chatllm
{
    ...
    std::string ggml::type_to_str(type t)
    {
        ...
        return std::string(traits->type_name);
    }
    ...
    {
        a->set_flag(GGML_TENSOR_FLAG_INPUT);
    }

    void ggml::set_output(ggml::tensor* a)
    {
        a->set_flag(GGML_TENSOR_FLAG_OUTPUT);
    }
    ...
    std::string_view ggml::get_name(tensor* tensor)
    {
    ...
    float ggml::at(ggml::tensor* a, int64_t i, int64_t j, int64_t k, int64_t l)
    {
        ...
        case ggml::type::GGML_TYPE_F16:
            return toFloat32(*(ggml_fp16_t*)p);
    ...
    ggml::tensor* ggml::inplace_act(ComputeContext* ctx, ActFunc act, ggml::tensor* input)
    {
        ...
        case ActFunc::GELU_QUICK:
            tensor = ggml_gelu_quick(ctx->get_ctx(), input, true);
            ctx->cb_op_tensor(tensor);
            break;
        case ActFunc::SILU:
    ...
    ggml::tensor* ggml::act(ComputeContext* ctx, ActFunc act, ggml::tensor* input)
    {
        ...
        case ActFunc::GELU:
            ...
            break;
        case ActFunc::GELU_QUICK:
            tensor = ggml_gelu_quick(ctx->get_ctx(), input, false);
            ctx->cb_op_tensor(tensor);
            break;
        case ActFunc::SILU:
    ...
    ggml::tensor* ggml::cast_int_to_i64(ComputeContext* ctx, ggml::tensor* a)
    {
        ...
        return custom(ctx, ggml::type::GGML_TYPE_I64,
            { ggml::get_dim(a, 0), ggml::get_dim(a, 1), ggml::get_dim(a, 2), ggml::get_dim(a, 3) }, { a }, ggml_custom_int_to_i64);
    }
