...
static task_vector ggml_compute_forward_mat_scale_f32(ggml::tensor* dst)
{
...
task_vector ggml_compute_forward_mat_scale(ggml::tensor* dst)
{
    const ggml::tensor* a = dst->src[0];
    switch (a->type)
    {
    case GGML_TYPE_F32:
        return ggml_compute_forward_mat_scale_f32(dst);
    default:
        std::unreachable();
    }
}

static task_vector ggml_compute_forward_ntk_dynamic_rope_f32(ggml::tensor* dst, const QWenSelfAttention* data)
{
    const ggml::tensor* src0 = dst->src[0];
    const ggml::tensor* src1 = dst->src[1];
...
static task_vector ggml_compute_forward_ntk_dynamic_rope_f16(ggml::tensor* dst, const QWenSelfAttention* data)
{
    const ggml::tensor* src0 = dst->src[0];
    const ggml::tensor* src1 = dst->src[1];
...
struct ggml_compute_forward_ntk_dynamic_rope {
    ...
    task_vector operator()(ggml::tensor* dst)
    {
        const ggml_tensor* a = dst->src[0];
        switch (a->type)
        {
        case GGML_TYPE_F16:
            return ggml_compute_forward_ntk_dynamic_rope_f16(dst, data);
        case GGML_TYPE_F32:
            return ggml_compute_forward_ntk_dynamic_rope_f32(dst, data);
        default:
            std::unreachable();
        }
    }
...
static task_vector ggml_compute_forward_ntk_mix_rope_f32(ggml::tensor* dst, const BlueLMSelfAttention* data)
{
    const ggml::tensor* src0 = dst->src[0];
    const ggml::tensor* src1 = dst->src[1];
...
static task_vector ggml_compute_forward_ntk_mix_rope_f16(ggml::tensor* dst, const BlueLMSelfAttention* data)
{
    const ggml::tensor* src0 = dst->src[0];
    const ggml::tensor* src1 = dst->src[1];
...
struct ggml_compute_forward_ntk_mix_rope {
    ...
    task_vector operator()(ggml::tensor* dst)
    {
        ...
        {
        case GGML_TYPE_F16:
            return ggml_compute_forward_ntk_mix_rope_f16(dst, data);
        case GGML_TYPE_F32:
            return ggml_compute_forward_ntk_mix_rope_f32(dst, data);
        default:
            std::unreachable();
        }
    }
...
static task_vector ggml_compute_forward_chatglm1_rope_f16(ggml::tensor* dst, const GLMSelfAttention* data)
{
    const ggml::tensor* src0 = dst->src[0];
    const ggml::tensor* src1 = dst->src[1];
...
static task_vector ggml_compute_forward_chatglm1_rope_f32(ggml::tensor* dst, const GLMSelfAttention* data)
{
    const ggml::tensor* src0 = dst->src[0];
    const ggml::tensor* src1 = dst->src[1];
...
struct ggml_compute_forward_chatglm1_rope {
    ...
    task_vector operator()(ggml::tensor* dst)
    {
        ...
        {
        case GGML_TYPE_F16:
            return ggml_compute_forward_chatglm1_rope_f16(dst, data);
        case GGML_TYPE_F32:
            return ggml_compute_forward_chatglm1_rope_f32(dst, data);
        default:
            std::unreachable();
        }
    }
...
static task_vector ggml_compute_forward_randn_f32(ggml::tensor* dst)
{
    const int nr = (int)ggml::nrows(dst);
...
static task_vector ggml_compute_forward_randn_f16(ggml::tensor* dst)
{
    const int nr = (int)ggml::nrows(dst);
...
task_vector ggml_compute_forward_randn(ggml::tensor* dst)
{
    switch (dst->type)
    {
    case GGML_TYPE_F16:
        return ggml_compute_forward_randn_f16(dst);
    case GGML_TYPE_F32:
        return ggml_compute_forward_randn_f32(dst);
        break;
    default:
        std::unreachable();
    }
...
task_vector ggml_custom_compute_forward_randn(ggml_tensor* dst)
{
    switch (dst->type)
    {
    case GGML_TYPE_F16:
...
template <class T> 
static task_vector ggml_compute_forward_flip_T_0(ggml::tensor* dst, const ggml::tensor* a)
{
...
struct ggml_compute_forward_flip {
    ...
    task_vector operator()(ggml::tensor* dst) {
        ...
        {
        case GGML_TYPE_F16:
            return ggml_compute_forward_flip_T_0<ggml_fp16_t>(dst, a);
        case GGML_TYPE_F32:
            return ggml_compute_forward_flip_T_0<float>(dst, a);
        default:
            GGML_ASSERT(false);
            break;
        }
...
task_vector ggml_custom_compute_forward_zeroes(ggml_tensor* dst)
{
    task_vector tasks;
    tasks.emplace_back([=] {
...
static task_vector ggml_compute_forward_bicubic_f32(ggml::tensor* dst, const ggml::tensor* src0, bool align_corners)
{
    task_vector tasks;
    tasks.emplace_back([=] {
        float f[4][4];
...
struct ggml_compute_forward_bicubic {
    ...
    task_vector operator()(ggml_tensor* dst)
    {
        ...
        case GGML_TYPE_F32:
            ...
            return ggml_compute_forward_bicubic_f32(dst, dst->src[0], align_corners);
        default:
            std::unreachable();
        }
    }
...
task_vector ggml_custom_merge_patch_1(ggml_tensor* dst, const ggml_tensor* src, const ggml::merge_patch_param* param)
{
...
struct ggml_custom_merge_patch {
    ...
    task_vector operator()(ggml_tensor* dst)
    {
        ...
        return ggml_custom_merge_patch_1(dst, a, param);
    }
};
...
struct ggml_custom_xielu
{
    ...
    float beta;
    float eps;
    task_vector xielu_f32(ggml_tensor* dst, const ggml_tensor* src0)
    {
        const int64_t nr = ggml::nrows(dst);
        const int64_t ir0 = 0;
        const int64_t ir1 = nr;
...
static task_vector ggml_custom_int_div_inner(ggml_tensor* dst, const ggml_tensor* src0, const int b)
{
    const int64_t nr = ggml::nrows(dst);
    const int64_t ir0 = 0;
    const int64_t ir1 = nr;
...
struct ggml_custom_int_div {
    ...
    task_vector operator()(ggml_tensor* dst)
    {
        ...
        case GGML_TYPE_I32:
            ...
            return ggml_custom_int_div_inner(dst, src0, b);
        default:
            std::unreachable();
        }
    }
...
template <class T> 
static task_vector ggml_custom_int_T_to_i64(ggml_tensor* dst, const ggml_tensor* src0)
{
    const int64_t nr = ggml::nrows(dst);
    const int64_t ir0 = 0;
    const int64_t ir1 = nr;
...
task_vector ggml_custom_int_to_i64(ggml_tensor* dst)
{
    ggml_tensor* a = dst->src[0];
    switch (a->type)
    {
    case GGML_TYPE_I32:
        return ggml_custom_int_T_to_i64<int32_t>(dst, a);
    case GGML_TYPE_I16:
        return ggml_custom_int_T_to_i64<int16_t>(dst, a);
    default:
        std::unreachable();
    }
}
