...
static constexpr int    RDMA_RX_DEPTH = 24;            // pre-posted recv ring: 24 กั 256 KiB = 6 MiB
...
struct rdma_conn {
    ...
    void* rx_buf = nullptr; // RDMA_RX_DEPTH กั RDMA_CHUNK contiguous
...
bool socket_t::impl::rdma_probe() {
    ...
    for (int d = 0; d < num_devs; d++) {
        ...
        if (found_gid < 0) {
            ...
            // Explicit GID index from GGML_RDMA_GID กX fetch its type for logging.
            ibv_gid_entry entry = {};
