...
static int iq3_find_best_neighbour(const uint16_t* neighbours, const uint32_t* grid,
    const float* xval, const float* weight, float scale, int8_t* L) {
    int num_neighbors = neighbours[0];
    assert(num_neighbors > 0);
    float best_d2 = FLT_MAX;
...
static int iq1_find_best_neighbour2(const uint16_t* neighbours, const uint64_t* grid,
...
static void quantize_row_iq1_m_impl(const float* x, block_iq1_m* y, int64_t n, const float* quant_weights,
    ...
    for (int ibl = 0; ibl < nbl; ++ibl) {
        ...
        s.f16 = std::bit_cast<uint16_t>(fromFloat32<ggml_fp16_t>(d * 1.1125f)); // 1.1125f is another fudge factor. Don't ask me why it is needed.
