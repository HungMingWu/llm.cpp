...
static void quantize_row_iq2_xxs_impl(const float* x, block_iq2_xxs* y, int64_t n, const float* quant_weights) {
    ...
    assert(!kgrid_q2xs.empty() && "forgot to call ggml_quantize_init()?");
    assert(!kmap_q2xs.empty() && "forgot to call ggml_quantize_init()?");
    assert(!kneighbors_q2xs.empty() && "forgot to call ggml_quantize_init()?");
    assert(n % QK_K == 0);
...
static void quantize_row_iq3_xxs_impl(int grid_size, const float* x, void* vy, int64_t n,
    ...
    assert(!kgrid_q3xs.empty() && "forgot to call ggml_quantize_init()?");
    assert(!kmap_q3xs.empty() && "forgot to call ggml_quantize_init()?");
    assert(!kneighbors_q3xs.empty() && "forgot to call ggml_quantize_init()?");
    assert(n % QK_K == 0);
...
void ggml_quantize_init(ggml_type type)
{
    ...
    std::lock_guard l(ggml_critical_section_mutex);
