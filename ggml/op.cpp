...
ggml_tensor* ggml_reshape_2d(
	...
	int64_t ne1) {
	GGML_ASSERT(ggml_is_contiguous(a));
	GGML_ASSERT(a->nelements() == ne0 * ne1);

	struct ggml_tensor* result = ctx->create(a->type, { ne0, ne1 }, a, 0);
...
ggml_tensor* ggml_reshape_3d(
	...
	int64_t ne2) {
	GGML_ASSERT(ggml_is_contiguous(a));
	GGML_ASSERT(a->nelements() == ne0 * ne1 * ne2);

	ggml_tensor* result = ctx->create(a->type, { ne0, ne1, ne2 }, a, 0);
	result->set_name(std::format("{} (reshaped)", a->name));
...
ggml_tensor* ggml_cont_4d(
	...
	int64_t ne2,
	int64_t ne3) {
	GGML_ASSERT(a->nelements() == (ne0 * ne1 * ne2 * ne3));
