...
struct test_cpy : public test_case {
    ...
    std::string vars() override {
        return std::format("type_src={},type_dst={},ne={},permute_src={},permute_dst={},_src_transpose={}",
            type_src, type_dst, ne, permute_src, permute_dst, static_cast<int>(_src_transpose));
    }
...
static bool test_backend(ggml_backend* backend, test_mode mode, const char* op_names_filter, const char* params_filter,
    ...
    if (mode == MODE_TEST) {
        ...
        for (auto& test : test_cases) {
            test_status_t status = test->eval(backend, backend_cpu.get(), op_names_filter, output_printer);
...
int main(int argc, char** argv) {
    ...
    bool ok = test_backend(backend.get(), mode, op_names_filter, params_filter, output_printer.get(), test_file_path);
