...
struct console_printer : public printer {
    ...
    void print_operation(const test_operation_info& info) override {
        ...
        if (info.has_gradient_info) {
            std::print("[{}] nonfinite gradient at index {} ({}={:f}) ", info.op_name, info.gradient_index,
                info.gradient_param_name, info.gradient_value);
        }
...
private:
    void print_test_console(const test_result& result) {
        std::print("  {}({}): ", result.op_name, result.op_params);
        if (!result.supported) {
            std::println("not supported [{}] ", result.backend_name);
            return;
        }

        if (result.passed) {
            std::println("\033[1;32mOK\033[0m");
        }
        else {
            std::println("\033[1;31mFAIL\033[0m");
        }
    }
...
static bool backend_has_feature(ggml_backend* backend, const char* feature_name) {
    ggml_backend_device* dev = backend->get_device();
    ggml_backend_reg* reg = dev->get_backend_reg();

    for (const ggml_backend_feature &f : reg->get_features()) {
        if (f.name == feature_name && f.value == "1") {
            return true;
        }
    }
    return false;
...
static void init_tensor_kq_mask(ggml_tensor* tensor, float min = -1.0f, float max = 1.0f) {
    ...
    std::vector<float>       data_f32(tensor->ne[0] * tensor->ne[1] * tensor->ne[2] * tensor->ne[3]);
    std::vector<ggml_fp16_t> data_f16(tensor->ne[0] * tensor->ne[1] * tensor->ne[2] * tensor->ne[3]);
    ...
    const int n_inf_zero_blocks = 0.2 * (tensor->ne[0] * tensor->ne[1] * tensor->ne[2] * tensor->ne[3]) / (blck0 * blck1);

    for (int b = 0; b < n_inf_zero_blocks; b++) {
        const int p3 = (rd() % tensor->ne[3]);
        const int p2 = (rd() % tensor->ne[2]);
        const int p1 = (rd() % tensor->ne[1]);
        const int p0 = (rd() % tensor->ne[0]);

        bool inf = rd() & 1;

        for (int i1 = 0; i1 < blck1 && p1 + i1 < tensor->ne[1]; i1++) {
            const int idx = p3 * tensor->ne[2] * tensor->ne[1] * tensor->ne[0] + p2 * tensor->ne[1] * tensor->ne[0] + (p1 + i1) * tensor->ne[0] + p0;

            for (int i0 = 0; i0 < blck0 && p0 + i0 < tensor->ne[0]; i0++) {
                data_f32[idx + i0] = inf ? -INFINITY : 0.0f;
            }
        }
    }

    for (size_t i = 0; i < tensor->ne[0] * tensor->ne[1] * tensor->ne[2] * tensor->ne[3]; i++)
		data_f16[i] = fromFloat32<ggml_fp16_t>(data_f32[i]);
...
static void init_tensor_tril(ggml_tensor* tensor, float min = -1.0f, float max = 1.0f) {
    ...
    ggml_backend_tensor_set(tensor, data_f32.data(), 0, tensor->nbytes());
}
...
struct test_case {
    ...
    virtual double max_err(ggml_backend* backend) {
    ...
    test_status_t eval(ggml_backend* backend1,
        ggml_backend* backend2,
        ...
        ggml_tensor* out = build_graph(&ctx, use_weights ? &ctx_weights : nullptr);
            ...
            for (auto& t : ctx.getTensors()) {
                if (!backend->get_device()->supports_op(t)) {
                    ...
                    if (unsupported_str.empty()) {
                        unsupported_str = std::string(backend->get_name());
                    }
                    else {
                        unsupported_str += ", " + std::string(backend->get_name());
                    }
        ...
        std::unique_ptr<ggml_backend_buffer> buf_weights;
        if (use_weights) {
            buf_weights = backend1->alloc_tensors(&ctx_weights);
            if (!buf_weights) {
                printf("failed to allocate weight tensors [%s] ", backend1->get_name());
                return test_status_t::FAIL;
            }
            buf_weights->setUsage(GGML_BACKEND_BUFFER_USAGE_WEIGHTS);
        }
        ...
        auto callback = [&](ggml_tensor* t1, ggml_tensor* t2) -> bool {
            ...
            double err = this->err(f1.data(), f2.data(), f1.size());
            if (err > this->max_err(backend1)) {
                printf("[%s] ERR = %.9f > %.9f ", ggml_op_desc(t1), err, this->max_err(backend1));
        ...
        const bool cmp_ok = ggml_backend_compare_graph_backend(backend1, backend2, &gf, callback, fused_nodes_to_verify);

        // Create test result
        bool        test_passed = cmp_ok;
        std::string error_msg = test_passed ? "" : (!cmp_ok ? "compare failed" : "test failed");
        test_result result(backend1->get_name(), current_op_name, vars(), "test", supported, test_passed,
            error_msg);
    ...
    bool eval_perf(ggml_backend* backend, const char* op_names_filter, printer* output_printer) {
        ...
        if (!backend->supports_op(out)) {
            // Create test result for unsupported performance test
            test_result result(backend->get_name(), current_op_name, vars(), "perf", false, false,
                "not supported");
        ...
        std::unique_ptr<ggml_backend_buffer> buf_weights;
        if (use_weights) {
            buf_weights = backend->alloc_tensors(&ctx_weights);
            if (!buf_weights) {
        ...
        int n_runs;
        bool is_cpu = backend->get_device()->get_type() == GGML_BACKEND_DEVICE_TYPE_CPU;
        if (op_flops(out) > 0) {
            // based on flops
            ...
            n_runs = std::min<int>(gf.getGraphSize() - gf.getNodes().size(), target_flops / op_flops(out)) + 1;
        }
        else {
            ...
            n_runs = std::min<int>(gf.getGraphSize() - gf.getNodes().size(), target_size / op_size(out)) + 1;
        }
        ...
        for (auto node : gf.getNodes()) {
            if (ggml_is_view_op(node->op) || node == out) {
                continue;
            }
            mem += tensor_op_size(node);
        }
        ...
        test_result result(backend->get_name(), current_op_name, vars(), "perf", true, true, "", avg_time_us,
    ...
    bool eval_support(ggml_backend* backend, const char* op_names_filter, printer* output_printer) {
        ...
        // TODO
        // gf = ggml_cgraph(ctx.get(), graph_nodes, false);
        ...
        bool supported = backend->supports_op(out);

        std::string device_desc = backend->get_device()->get_description();
        // TODO
        std::string backend_reg_name; // ggml_backend_reg_name(ggml_backend_dev_backend_reg(backend->get_device()));

        test_result result(backend->get_name(), current_op_name, vars(), "support", supported, supported,
    ...
    bool eval_grad(ggml_backend* backend, const char* op_names_filter, printer* output_printer) {
        ...
        out->set_flag(GGML_TENSOR_FLAG_LOSS);
        ...
        for (ggml_tensor* t : ctx.getTensors()) {
            ...
            std::vector<float> ga;
            ggml_tensor* grad = ggml_graph_get_grad(&gb, t);
            if (grad) {
            ...
            for (int64_t i = 0; i < ne; ++i) {
                ...
                if (grad_precise()) {
                    ...
                    status = backend->graph_compute(&gf);
                    if (status != GGML_STATUS_SUCCESS) {
                        fprintf(stderr, "%s: backend->graph_compute failed. status=%s \n", __func__, ggml_status_to_string(status));
                        return false;
                    }
...
struct std::formatter<ggml_scale_mode> : std::formatter<std::string> {
    auto format(const ggml_scale_mode& mode, std::format_context& ctx) const {
        std::string str = [=] {
            int scale = mode & 0xFF;
            if (scale == GGML_SCALE_MODE_NEAREST) return std::string("nearest");
            else if (scale == GGML_SCALE_MODE_BILINEAR) return std::string("bilinear");
            else if (scale == GGML_SCALE_MODE_BICUBIC) return std::string("bicubic");
            else return std::to_string(mode);
        }();
...
template <>
struct std::formatter<ggml_glu_op> : std::formatter<std::string> {
    auto format(const ggml_glu_op& op, std::format_context& ctx) const {
        return std::format_to(ctx.out(), "{}", static_cast<int>(op));
    }
};
...
struct test_glu : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne_a={},v={},swapped={}", type, ne_a, v, (int)swapped);
    }
...
struct test_glu_split : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne_a={},v={},split", type, ne_a, v);
    }
...
struct test_swiglu_oai : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne_a={},v={},alpha={:6f},limit={:6f}", type, ne_a, v, alpha, limit);
    }
...
struct test_swiglu_clamp : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne_a={},v={},limit={:6f}", type, ne_a, v, limit);
    }
...
struct test_get_rows : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},n={},m={},r={},be1={},be2={},v={}",
            type, n, m, r, be1, be2, static_cast<int>(v));
    }
...
struct test_pool1d : public test_case {
    ...
    std::string vars() override {
        return std::format("pool_type={},type_input={},ne_input={},k0={},s0={},p0={}",
            pool_type, type_input, ne_input, k0, s0, p0);
    }
...
struct test_set_rows : public test_case {
    ...
    std::string vars() override {
        return std::format("type_src={},type_dst={},type_idx={},ne={},nr23={},r={},v={}",
            type_src, type_dst, type_idx, ne, nr23, r, static_cast<int>(v));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ggml_tensor* dst = ggml_new_tensor(ctx, type_dst, ne[0], ne[1], ne[2] * nr23[0], ne[3] * nr23[1]);
        dst->set_name("dst");

        ggml_tensor* src = ggml_new_tensor(ctx, type_src, ne[0], r, ne[2] * nr23[0], ne[3] * nr23[1]);
        src->set_name("src");
        ...
        if (v) {
            src = ggml_view(ctx, src, { ne[0], r / 2, ne[2] * nr23[0], ne[3] * nr23[1] }, { src->nb[1], src->nb[2], src->nb[3] }, 0);
            row_idxs = ggml_view(ctx, row_idxs, { r / 2, ne[2], ne[3] }, { row_idxs->nb[1], row_idxs->nb[2] }, 0);
            row_idxs->set_name("view_of_rows");
        }
...
struct test_im2col_3d : public test_case {
    ...
    std::string vars() override {
        return std::format("type_input={},type_kernel={},dst_type={},ne_input={},ne_kernel={},IC={}"
            ",s0={},s1={},s2={},p0={},p1={},p2={},d0={},d1={},d2={},v={}",
            type_input, type_kernel, dst_type,
            ne_input, ne_kernel, IC, s0, s1, s2, p0, p1, p2, d0, d1, d2, static_cast<int>(v));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (v) {
            input = ggml_view(ctx, input, { ne_input[0] - 2, ne_input[1] - 2, ne_input[2] - 2, ne_input[3] - 2 }, { input->nb[1], input->nb[2], input->nb[3] }, 0);
...
struct test_conv_2d_dw : public test_case {
    ...
    std::string vars() override {
        return std::format("ne_input={},ne_kernel={},type_kernel={},stride={},padding={},dilation={},cwhn={}",
            ne_input, ne_kernel, type_kernel, stride, padding, dilation, static_cast<int>(cwhn));
    }
...
struct test_conv_3d : public test_case {
    ...
    std::string vars() override {
        return std::format("N={},IC={},ID={},IH={},IW={},OC={},KD={},KH={},KW={}"
            ",s0={},s1={},s2={},p0={},p1={},p2={},d0={},d1={},d2={},type_kernel={}",
            N, IC, ID, IH, IW, OC, KD, KH, KW,
            s0, s1, s2, p0, p1, p2, d0, d1, d2, type_kernel);
    }
...
struct test_col2im_1d : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},K={},OC={},T_in={},s0={},p0={}",
            type, K, OC, T_in, s0, p0);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ggml_tensor* cols = ggml_new_tensor(ctx, type, K * OC, T_in);
        cols->set_name("cols");
...
struct test_conv_transpose_2d : public test_case {
    ...
    std::string vars() override {
        return std::format("kernel_type={},ne_input={},ne_kernel={},stride={}",
            kernel_type, ne_input, ne_kernel, stride);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* kernel = ggml_new_tensor(ctx, kernel_type, ne_kernel);
        kernel->set_name("kernel");
...
struct test_rope_set_rows : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},type_idx={},ne_a={},mode={}", type, type_idx, ne_a, mode);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (is_mrope) {
            if (is_vision) {
                ...
                rope = ggml_rope_multi(ctx, a, pos, freq, n_dims / 2, rope_sections, mode, 0, 10000.0f, fs, ef, af, 1.0f, 1.0f, false);
            }
            else {
                GGML_ASSERT(n_dims / 3 > 0);
                int rope_sections[4] = { n_dims / 3, n_dims / 3, n_dims / 3, 0 };
                rope = ggml_rope_multi(ctx, a, pos, freq, n_dims, rope_sections, mode, 0, 10000.0f, fs, ef, af, 1.0f, 1.0f, false);
            }
        }
        else {
            rope = ggml_rope(ctx, a, pos, ne_a[0], mode, false);
        }
...
struct test_rms_norm_mul_rope : public test_case {
    ...
    std::string vars() override {
        return std::format("ne={},eps={:6f},multi_add={},set_rows={},broadcast={},mode={}", ne, eps,
            static_cast<int>(multi_add), static_cast<int>(set_rows), static_cast<int>(broadcast), mode);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* w = broadcast ? ggml_new_tensor(ctx, GGML_TYPE_F32, ne[0]) : b;

        a = ggml_mul(ctx, ggml_rms_norm(ctx, a, eps, false), w, false);
...
struct test_dup : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_dup(ctx, src, false);
        out->set_name("out");
...
struct test_set : public test_case {
    ...
    std::string vars() override {
        return std::format("type_src={},type_dst={},ne={},dim={},inplace={}",
            type_src, type_dst, ne, dim, static_cast<int>(inplace));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_set(ctx, dst, src,
            ...
            src->nb[1], src->nb[2], src->nb[3], offset, inplace);
        out->set_name("out");
...
struct test_cpy : public test_case {
    ...
    std::string vars() override {
        if (_use_dst_alloc) {
            return std::format("type_src={},type_dst={},ne_src={},ne_dst={},permute_src={},permute_dst={},_src_transpose={},dst_alloc={}",
                type_src, type_dst, ne_src, ne_dst, permute_src, permute_dst, static_cast<int>(_src_transpose), dst_alloc);
        }
        if (_use_dst_shape) {
            return std::format("type_src={},type_dst={},ne_src={},ne_dst={},permute_src={},permute_dst={},_src_transpose={}",
                type_src, type_dst, ne_src, ne_dst, permute_src, permute_dst, static_cast<int>(_src_transpose));
        }
        return std::format("type_src={},type_dst={},ne_src={},permute_src={},permute_dst={},_src_transpose={}",
            type_src, type_dst, ne_src, permute_src, permute_dst, static_cast<int>(_src_transpose));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ggml_tensor* src = ggml_new_tensor(ctx, type_src, ne_src);
        src->set_flag(GGML_TENSOR_FLAG_PARAM);
        src->set_name("src");
        ...
        std::array<int64_t, 4> dst_ne = _use_dst_shape ? ne_dst : src->ne;
        ggml_tensor* dst;

        if (_use_dst_alloc) {
            // view a sub-block of a larger buffer -> strided dst
            ggml_tensor* dst_buf = ggml_new_tensor(ctx, type_dst, dst_alloc);
            dst_buf->set_name("dst_buf");
            dst = ggml_view(ctx, dst_buf, { dst_ne[0], dst_ne[1], dst_ne[2], dst_ne[3] },
                { dst_buf->nb[1], dst_buf->nb[2], dst_buf->nb[3] }, 0);
            dst->set_name("dst_view");
        }
        else {
            dst = ggml_new_tensor(ctx, type_dst, dst_ne);
            dst->set_name("dst");
...
struct test_cont : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},use_view_slice={},permute={}", type, ne, static_cast<int>(use_view_slice), permute);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (permuted) {
            ...
            dst->set_name("src_permuted");
        }
        else if (use_view_slice) {
            dst = ggml_view(ctx, src, { src->ne[0], 1, src->ne[2], src->ne[3] },
                { src->nb[1], src->nb[2], src->nb[3] }, src->nb[0] * (src->ne[1] - 1));
            dst->set_name("src_view_slice");
        }
        else {
            dst = ggml_transpose(ctx, src);
            dst->set_name("src_transposed");
        }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        for (int i = 0; i < nf; ++i) {
            if (perm1) {
                ...
                b[i] = ggml_new_tensor(ctx, type, ne[p[0]], ne[p[1]], ne[p[2]], ne[p[3]]);
                b[i] = ggml_permute(ctx, b[i], p[0], p[1], p[2], p[3]);
            }
            else if (src_overlap) {
                b[i] = ggml_view(ctx, a, { ne[0], ne[1], ne[2], 2 * (ne[3] / 3) }, { a->nb[1], a->nb[2], a->nb[3] }, (ne[3] / 3) * a->nb[3]);
            }
            else {
        ...
        if (grad_supported) {
            a->set_flag(GGML_TENSOR_FLAG_PARAM);
            b[0]->set_flag(GGML_TENSOR_FLAG_PARAM);
        }

        ggml_tensor* out;

        if (src_overlap) {
            out = ggml_view(ctx, a, { ne[0], ne[1], ne[2], 2 * (ne[3] / 3) }, { a->nb[1], a->nb[2], a->nb[3] }, 0);
        }
        else {
            out = a;
        }
...
struct test_add_id : public test_case {
    ...
    std::string vars() override {
        return std::format("type_a={},type_b={},n_embd={},n_experts={},n_experts_used={},n_token={}", 
            type_a, type_b, n_embd, n_experts, n_experts_used, n_token);
    }

    size_t op_size(ggml_tensor* t) override {
        return t->nbytes() + t->src[0]->nbytes() + t->src[2]->nbytes();
    }
...
struct test_scale : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},scale={:6f},bias={:6f},inplace={}", type, ne, scale, bias, static_cast<int>(inplace));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_scale_bias(ctx, a, scale, bias, inplace);
        out->set_name("out");
...
struct test_softcap : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},softcap={:.6f}", type, ne, softcap);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_scale(ctx, ggml_tanh(ctx, ggml_scale(ctx, a, 1.0f / softcap, false), false), softcap, false);
        out->set_name("out");
...
struct test_norm : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},v={},eps={:6f},noncontig_rows={}",
            type, ne, static_cast<int>(v), eps, static_cast<int>(noncontig_rows));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_norm(ctx, a, eps, false);
        out->set_name("out");
...
struct test_norm_mul_add : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        a = ggml_add(ctx, ggml_add(ctx, a, w, false), b, false);

        ggml_tensor* n = ggml_norm(ctx, a, eps, false);
        ggml_tensor* m = ggml_mul(ctx, n, w, false);
        ggml_tensor* out = ggml_add(ctx, m, b, false);
        out->set_name("out");
        return out;
    }
};
...
struct test_rms_norm : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},v={},eps={:6f},inplace={}", type, ne, static_cast<int>(v), eps, static_cast<int>(inplace));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor * out = ggml_rms_norm(ctx, a, eps, inplace);
        out->set_name("out");
...
struct test_rms_norm_mul_add : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        b->set_flag(GGML_TENSOR_FLAG_PARAM);
        b->set_name("b");
        c->set_flag(GGML_TENSOR_FLAG_PARAM);
        c->set_name("c");
        ...
        ggml_tensor* out = ggml_add(ctx, ggml_mul(ctx, ggml_rms_norm(ctx, a, eps, false), b, false), c, false);
        out->set_name("out");
...
struct test_add_rms_norm : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* add_result = ggml_add(ctx, a, b, false);
        add_result->set_name("add_result");

        ggml_tensor* out = ggml_rms_norm(ctx, add_result, eps, false);
        out->set_name("out");
...
struct test_l2_norm : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},eps={:6f},v={},noncontig_rows={}",
            type, ne, eps, static_cast<int>(v), static_cast<int>(noncontig_rows));
    }
...
struct test_unary_mul : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},swap={},layout={},tail={}", type, ne, static_cast<int>(swap), layout, tail);
    }
    ...
    ggml_tensor* padded(ggml_context* ctx, const char* name, int64_t mul0, int64_t off0) {
        ...
        ne_w[0] *= mul0;
        ggml_tensor* base = ggml_new_tensor(ctx, type, ne_w);
        base->set_name(name);
        return ggml_view(ctx, base, { ne[0], ne[1], ne[2], ne[3] },
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (layout == "packed") {
            ...
            ggml_tensor* base = ggml_new_tensor(ctx, type, ne_w);
            base->set_name("base");
            b = ggml_view(ctx, base, { ne[0], ne[1], ne[2], ne[3] }, { base->nb[1], base->nb[2], base->nb[3] }, 0);
            a = ggml_view(ctx, base, { ne[0], ne[1], ne[2], ne[3] }, { base->nb[1], base->nb[2], base->nb[3] },
                ne[0] * base->nb[0]);
        }
            ...
            ggml_tensor* base = ggml_new_tensor(ctx, type, ne_w);
            base->set_name("a");
            a = ggml_view(ctx, base, { ne[0], ne[1], ne[2], ne[3] }, { base->nb[1], base->nb[2], base->nb[3] }, 0);
            b = ggml_new_tensor(ctx, type, ne);
        }
        else if (layout == "bcast") {
            a = ggml_new_tensor(ctx, type, ne);
            b = ggml_new_tensor(ctx, type, ne[0], 1, 1, 1);
        }
        else {
...
struct test_snake_fuse : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ggml_tensor* x = ggml_new_tensor(ctx, type, ne);
        x->set_name("x");

        ggml_tensor* a = ggml_new_tensor(ctx, GGML_TYPE_F32, 1, ne[1]);
        a->set_name("a");

        ggml_tensor* inv_b = ggml_new_tensor(ctx, GGML_TYPE_F32, 1, ne[1]);
        inv_b->set_name("inv_b");

        // exact 5-op chain that BigVGAN / Vocos frontends emit
        ggml_tensor* ax = ggml_mul(ctx, x, a, false);
        ggml_tensor* sin_ax = ggml_sin(ctx, ax);
        ggml_tensor* sin_sq = ggml_sqr(ctx, sin_ax, false);
        ggml_tensor* scaled = ggml_mul(ctx, sin_sq, inv_b, false);
        ggml_tensor* out = ggml_add(ctx, x, scaled, false);
        out->set_name("out");
...
struct test_dsv4_hc : public test_case {
    ...
    void initialize_tensors(ggml_context* ctx) override {
        for (ggml_tensor* t : ctx->getTensors()) {
            ...
            std::vector<float> data(t->nelements());
            for (float& v : data) {
                v = dist(rng);
            }
...
struct test_dsv4_hc_comb : public test_dsv4_hc {
    ...
    std::string vars() override {
        return std::format("n_tokens={},n_iter={},eps={:6f}", n_tokens, n_iter, eps);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ggml_tensor* mixes = ggml_new_tensor(ctx, GGML_TYPE_F32, (2 + hc) * hc, n_tokens);
        mixes->set_name("mixes");

        ggml_tensor* scale = ggml_new_tensor(ctx, GGML_TYPE_F32, 3);
        scale->set_name("scale");

        ggml_tensor* base = ggml_new_tensor(ctx, GGML_TYPE_F32, (2 + hc) * hc);
        base->set_name("base");
...
struct test_dsv4_hc_pre : public test_dsv4_hc {
    ...
    std::string vars() override {
        return std::format("n_embd={},n_tokens={}", n_embd, n_tokens);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ggml_tensor* x = ggml_new_tensor(ctx, GGML_TYPE_F32, n_embd, hc, n_tokens);
        x->set_name("x");

        ggml_tensor* weights = ggml_new_tensor(ctx, GGML_TYPE_F32, hc, n_tokens);
...
struct test_dsv4_hc_post : public test_dsv4_hc {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ggml_tensor* x = ggml_new_tensor(ctx, GGML_TYPE_F32, n_embd, n_tokens);
        x->set_name("x");

        ggml_tensor* residual = ggml_new_tensor(ctx, GGML_TYPE_F32, n_embd, hc, n_tokens);
        residual->set_name("residual");

        ggml_tensor* post = ggml_new_tensor(ctx, GGML_TYPE_F32, hc, n_tokens);
        post->set_name("post");

        ggml_tensor* comb = ggml_new_tensor(ctx, GGML_TYPE_F32, hc, hc, n_tokens);
        comb->set_name("comb");
...
struct test_ssm_conv_bias_silu : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne_a={},ne_b={},fuse_bias={}", type, ne_a, ne_b, static_cast<int>(fuse_bias));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (fuse_bias) {
            ggml_tensor* bias = ggml_new_tensor(ctx, type, out->ne[0]);
            bias->set_name("bias");
            out = ggml_add(ctx, out, bias, false);
        }

        out = ggml_silu(ctx, out, false);
...
struct test_ssm_scan : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},d_state={},head_dim={},n_head={},n_group={},n_seq_tokens={},n_seqs={},xbc_overlap={},K={},weak_decay={}",
            type, d_state, head_dim, n_head, n_group, n_seq_tokens, n_seqs, static_cast<int>(xbc_overlap), K, static_cast<int>(weak_decay));
    }
...
struct test_ssm_scan_rollback : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},d_state={},head_dim={},n_head={},n_group={},n_seq_tokens={},n_seqs={},K={}",
            type, d_state, head_dim, n_head, n_group, n_seq_tokens, n_seqs, K);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* dt = ggml_new_tensor(ctx, type, n_head, n_seq_tokens, n_seqs);
        ggml_tensor* A = ggml_new_tensor(ctx, type, 1, n_head);
        ...
        for (int64_t slot = 0; slot < K; ++slot) {
            ...
            ggml_tensor* x_prefix = ggml_cont(ctx, ggml_view(ctx, x, { head_dim, n_head, prefix_tokens, n_seqs }, { x->nb[1], x->nb[2], x->nb[3] }, 0));
            ggml_tensor* dt_prefix = ggml_cont(ctx, ggml_view(ctx, dt, { n_head, prefix_tokens, n_seqs }, { dt->nb[1], dt->nb[2] }, 0));
            ggml_tensor* B_prefix = ggml_cont(ctx, ggml_view(ctx, B, { d_state, n_group, prefix_tokens, n_seqs }, { B->nb[1], B->nb[2], B->nb[3] }, 0));
            ggml_tensor* C_prefix = ggml_cont(ctx, ggml_view(ctx, C, { d_state, n_group, prefix_tokens, n_seqs }, { C->nb[1], C->nb[2], C->nb[3] }, 0));

            ggml_tensor* prefix = ggml_ssm_scan(ctx, s, x_prefix, dt_prefix, A, B_prefix, C_prefix, ids, /*K=*/1);

            ggml_tensor* full_state = ggml_view(ctx, full, { state_elems }, {}, (y_elems + slot * state_elems) * ggml_element_size(full));
            ggml_tensor* prefix_state = ggml_view(ctx, prefix, { state_elems }, {}, (head_dim * n_head * prefix_tokens * n_seqs) * ggml_element_size(prefix));
            ggml_tensor* diff = ggml_sum(ctx, ggml_sqr(ctx, ggml_sub(ctx, full_state, prefix_state, false), false));

            out = out == nullptr ? diff : ggml_add(ctx, out, diff, false);
        }
...
struct test_gated_delta_net : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (permuted) {
            ...
            q = ggml_permute(ctx, ggml_new_tensor(ctx, type, head_size, n_seq_tokens, head_count, n_seqs), 0, 2, 1, 3);
            k = ggml_permute(ctx, ggml_new_tensor(ctx, type, head_size, n_seq_tokens, head_count, n_seqs), 0, 2, 1, 3);
            v = ggml_permute(ctx, ggml_new_tensor(ctx, type, head_size, n_seq_tokens, head_count * v_repeat, n_seqs), 0, 2, 1, 3);
        }
        else {
            q = ggml_new_tensor(ctx, type, head_size, head_count, n_seq_tokens, n_seqs);
            k = ggml_new_tensor(ctx, type, head_size, head_count, n_seq_tokens, n_seqs);
            v = ggml_new_tensor(ctx, type, head_size, head_count * v_repeat, n_seq_tokens, n_seqs);
        }
        q->set_name("q");
        k->set_name("k");
        v->set_name("v");
        ...
        ggml_tensor* state = ggml_new_tensor(ctx, type, head_size, head_size, head_count * v_repeat, n_seqs);
        g->set_name("g");
        beta->set_name("beta");
...
struct test_mul_mat : public test_case {
    ...
    std::string vars() override {
        return std::format("type_a={},type_b={},m={},n={},k={},bs={},nr={},per={},k_v={},o={},src_overlap={}",
            type_a, type_b, m, n, k, bs, nr, per, k_v, o, static_cast<int>(src_overlap));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (npermuted > 0) {
            ...
            ggml_tensor* base = ggml_new_tensor(ctx, type_a, 2 * k, std::max(m, n), bs[0] * nr[0], bs[1] * nr[1]);
            base->set_name("base");

            a = ggml_view(ctx, base, { k, m, bs[0], bs[1] }, { base->nb[1], base->nb[2], base->nb[3] }, 0);
            b = ggml_view(ctx, base, { k, n, bs[0] * nr[0], bs[1] * nr[1] }, { base->nb[1], base->nb[2], base->nb[3] }, k * ggml_type_size(type_a));
            a->set_name("a");
            b->set_name("b");
        }
        else {
...
struct test_mul_mat_id_fusion : public test_case {
    ...
    std::string vars() override {
        return std::format("type_a={},type_b={},n_mats={},n_used={},b={},m={},n={},k={},o={},mul={}",
            type_a, type_b, n_mats, n_used, static_cast<int>(b), m, n, k, o, static_cast<int>(mul));
    }
...
struct test_sqr : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_sqr(ctx, a, false);
        out->set_name("out");
...
struct test_clamp : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_clamp(ctx, a, min, max, false);
        out->set_name("out");
...
struct test_floor : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_floor(ctx, a, false);
        out->set_name("out");
...
struct test_ceil : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_ceil(ctx, a, false);
        out->set_name("out");
...
struct test_round : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_round(ctx, a, false);
        out->set_name("out");
...
struct test_trunc : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_trunc(ctx, a, false);
        out->set_name("out");
...
struct test_diag_mask_inf : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_diag_mask_inf(ctx, a, n_past, false);
        out->set_name("out");
...
struct test_soft_max : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},mask={},sinks={},m_prec={},nr23={},scale={:6f},max_bias={:6f},inplace={}",
            type, ne, static_cast<int>(mask), static_cast<int>(sinks), m_prec, nr23, scale, max_bias, static_cast<int>(inplace));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_soft_max(ctx, a, inplace, mask, scale, max_bias);
...
struct test_rope : public test_case {
    ...
    std::string vars() override {
        ...
        return std::format("type={},ne_a={},n_dims={},mode={},n_ctx={},fs={:6f},ef={:6f},af={:6f},ff={},v={},inplace={},n_offs={}",
            type, ne_a, n_dims, mode, n_ctx, fs, ef, af, static_cast<int>(ff), static_cast<int>(v), static_cast<int>(inplace), n_offs);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (v & 1) {
            ...
            a = ggml_view(ctx, a, { ne_a[0], ne_a[1], ne_a[2], ne_a[3] },
        ...
        if (is_mrope) {
            if (is_vision) {
                ...
                int rope_sections[4] = { n_dims / 4, n_dims / 4, 0, 0 }; // Vision-RoPE only use first two dimension for image (x, y) coordinate
                if (forward) {
                    out = ggml_rope_multi(ctx, a, pos, freq, n_dims / 2, rope_sections, mode, 0, 10000.0f, fs, ef, af, 1.0f, 1.0f, inplace);
                }
                else {
                ...
                int rope_sections[4] = { n_dims / 3, n_dims / 3, n_dims / 3, 0 };
                if (forward) {
                    out = ggml_rope_multi(ctx, a, pos, freq, n_dims, rope_sections, mode, 0, 10000.0f, fs, ef, af, 1.0f, 1.0f, inplace);
                }
                else {
            ...
            if (forward) {
                out = ggml_rope_ext(ctx, a, pos, freq, n_dims, mode, 0, 10000.0f, fs, ef, af, 1.0f, 1.0f, inplace);
            }
            else {
...
struct test_top_k : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},k={},ties={}",
            type, ne, k, static_cast<int>(ties));
    }
    ...
    double err(const float* a, const float* b, size_t n) override {
        ...
        if (ties) {
            std::vector<float> src(input->nelements());

            ggml_backend_tensor_get(input, src.data(), 0, input->nelements() * ggml_type_size(type));
...
struct test_topk_moe : public test_case {
    ...
    std::string vars() override {
        return std::format("ne={},n_expert_used={},with_norm={},bias_probs={},gating_func={},scale_w={:6f}",
            ne, n_expert_used, static_cast<int>(with_norm), static_cast<int>(bias_probs), static_cast<int>(gating_func),
            scale_w);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* probs =
            ...
            (gating_func == GATING_FUNC_SIGMOID) ? ggml_sigmoid(ctx, logits, false) :
            (gating_func == GATING_FUNC_SQRT_SOFTPLUS) ? ggml_sqrt(ctx, ggml_softplus(ctx, logits, false)) : logits;
        ...
        selected_experts->set_name("selected_experts");

        weights = ggml_get_rows(ctx, ggml_reshape(ctx, probs, { 1, n_expert, n_tokens }), selected_experts); // [1, n_expert_used, n_tokens]
        weights->set_name("weights");

        if (gating_func == GATING_FUNC_SOFTMAX_WEIGHT) {
            weights = ggml_reshape(ctx, weights, { n_expert_used, n_tokens });
            weights = ggml_soft_max(ctx, weights, false);  // [n_expert_used, n_tokens]
        ...
        if (with_norm) {
            ...
            weights_sum->set_name("weights_sum");

            weights_sum = ggml_clamp(ctx, weights_sum, 6.103515625e-5, INFINITY, false);
            weights = ggml_div(ctx, weights, weights_sum, false); // [n_expert_used, n_tokens]
            weights = ggml_reshape(ctx, weights, { 1, n_expert_used, n_tokens });
        }

        if (scale_w) {
            weights = ggml_scale(ctx, weights, scale_w, false);
        }
...
struct test_mul_mat_vec_fusion : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},glu_op={},m={},n={},k={},use_id={},n_mats={},n_used={},b={},with_bias={},with_gate={},with_lane_scale={},batch_dims={}",
            type, glu_op, m, n, k, static_cast<int>(use_id), n_mats,
            n_used, static_cast<int>(b), static_cast<int>(with_bias), static_cast<int>(with_gate), static_cast<int>(with_lane_scale), batch_dims);
    }
    ...
    ggml_tensor* build_lane_scale_dense(ggml_context* ctx, ggml_tensor* out) {
        ggml_tensor* scale = ggml_new_tensor(ctx, GGML_TYPE_F32, 1);
        return ggml_mul(ctx, out, scale, false);
    }
    ...
    ggml_tensor* build_lane_scale_id(ggml_context* ctx, ggml_context* ctx_weights, ggml_tensor* out, ggml_tensor* ids) {
        ...
        ggml_tensor* scale = ggml_new_tensor(ctx_weights, GGML_TYPE_F32, n_mats);
        ggml_tensor* s = ggml_reshape(ctx, scale, { 1, n_mats, 1 });
        s = ggml_repeat_4d(ctx, s, 1, n_mats, m, 1);
        s = ggml_get_rows(ctx, s, ids);
        return ggml_mul(ctx, out, s, false);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx, ggml_context* ctx_weights) override {
        if (!use_id) {
            ...
            ggml_tensor* scale = ggml_new_tensor(ctx, out->type, scale_ne);
            out = ggml_mul(ctx, out, scale, false);
...
struct test_sum : public test_case {
    ...
    std::string vars() override {
        std::string vars = std::format("type={},ne={}", type, ne);
        if (_use_permute) {
            vars += std::format(",permute={}", permute);
		}
        return vars;
    }
...
struct test_sum_rows : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},permute={},slice={}", type, ne, static_cast<int>(permute), static_cast<int>(slice));
    }
...
struct test_acc : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne_a={},ne_b={},stride_dim={}", type, ne_a, ne_b, stride_dim);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (stride_dim == 1 || stride_dim == 2 || stride_dim == 3) {
            ...
            ggml_tensor* b_pad = ggml_new_tensor(ctx, type, ne_b_pad);
            b_pad->set_flag(GGML_TENSOR_FLAG_PARAM);
            b_pad->set_name("b_pad");
            // View that skips the first row, so b has a non-zero byte offset
            b = ggml_view(ctx, b_pad,
        ...
        ggml_tensor* out = ggml_acc(ctx, a, b, { (int32_t)a->nb[1], (int32_t)a->nb[2], (int32_t)a->nb[3], 0 }, false);
        out->set_name("out");
...
struct test_pad : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne_a={},pad_0={},pad_1={},circular={}",
            type, ne_a, pad_0, pad_1, static_cast<int>(circular));
    }
...
struct test_pad_ext : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne_a={},lp0={},rp0={},lp1={},rp1={},lp2={},rp2={},lp3={},rp3={},tfrm={},circular={}",
            type, ne_a, lp0, rp0, lp1, rp1, lp2, rp2, lp3, rp3, tfrm, static_cast<int>(circular));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (tfrm == 1) {
            a = ggml_view(ctx, a, { (a->ne[0] + 1) / 2, (a->ne[1] + 1) / 2, (a->ne[2] + 1) / 2, (a->ne[3] + 1) / 2 }, { a->nb[1], a->nb[2], a->nb[3] }, 0);
            a->set_name("view of a");
        } else if (tfrm == 2) {
        ...
        ggml_tensor* out = ggml_pad_ext(ctx, a, lp0, rp0, lp1, rp1, lp2, rp2, lp3, rp3, circular);
        out->set_name("out");
...
struct test_roll : public test_case {
    ...
    std::string vars() override {
        return std::format("shift0={},shift1={},shift3={},shift4={},permute={}",
            shift0, shift1, shift3, shift4, static_cast<int>(permute));
    }
...
struct test_flash_attn_ext : public test_case {
    ...
    std::string vars() override {
        return std::format("hsk={},hsv={},nh={},nr23={},kv={},nb={},mask={},sinks={},max_bias={:6f},"
            "logit_softcap={:6f},prec={},type_K={},type_V={},permute={},kv_view={},v_is_view_of_k={}", hsk, hsv,
            nh, nr23, kv, nb, static_cast<int>(mask), static_cast<int>(sinks),
            max_bias, logit_softcap, prec, type_K, type_V, permute, static_cast<int>(kv_view), static_cast<int>(v_is_view_of_k));
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        if (v_is_view_of_k) {
            ...
            v = ggml_view(ctx, k, { hsv_padded, kv, nh, nr23[1] }, { k->nb[1], k->nb[2], k->nb[3] }, 0);
        }
        else {
...
struct test_cross_entropy_loss : public test_case {
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        labels = ggml_soft_max(ctx, labels, false);
...
struct test_tri : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},tri_type={}", type, ne, static_cast<int>(tri_type));
    }
...
struct test_fill : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne={},c={:6f}", type, ne, c);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        ggml_tensor* out = ggml_fill(ctx, a, c, false);
...
struct test_solve_tri : public test_case {
    ...
    std::string vars() override {
        return std::format("type={},ne_lhs={},ne_rhs={}", type, ne_lhs, ne_rhs);
    }
...
struct test_lightning_indexer : public test_case {
    ...
    std::string vars() override {
        return std::format("hsk={},nh={},kv={},nb={},ns={},nm={},type_K={}", hsk, nh, kv, nb, ns, nm, type_K);
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ggml_tensor* q = ggml_new_tensor(ctx, GGML_TYPE_F32, hsk, nh, nb, ns);
        q->set_flag(GGML_TENSOR_FLAG_PARAM);
        q->set_name("q");

        ggml_tensor* k = ggml_new_tensor(ctx, type_K, hsk, 1, kv, ns);
        k->set_flag(GGML_TENSOR_FLAG_PARAM);
        k->set_name("k");

        ggml_tensor* w = ggml_new_tensor(ctx, GGML_TYPE_F32, nh, nb, 1, ns);
        w->set_flag(GGML_TENSOR_FLAG_PARAM);
        w->set_name("w");

        ggml_tensor* m = ggml_new_tensor(ctx, GGML_TYPE_F16, kv, nb, 1, nm);
        m->set_flag(GGML_TENSOR_FLAG_PARAM);
        m->set_name("m");
...
#if 0
        if (name.empty()) {
            return std::format("type={},ne={},op_params={},sources={}", type, ne, op_params, sources);
        }

        return std::format("name={},type={},ne={},op_params={},sources={}", name, type, ne, op_params, sources);
#endif
        return "";
    }
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        for (size_t i = 0; i < source_count; ++i) {
            ...
            if (is_non_contiguous(src)) {
                ...
                ggml_tensor* backing = ggml_new_tensor(ctx, src.type, backing_elements);
                source_tensors[i] = ggml_view(ctx, backing,
                ...
                source_tensors[i] = ggml_new_tensor(ctx, src.type, src.ne);
            }
        }
        ...
        if (inplace && source_count > 0) {
            ...
            out = ggml_new_tensor(ctx, type, ne[0], ne[1], ne[2], ne[3]);
        }
        out->op = op;
        for (size_t i = 0; i < source_count; ++i) {
        ...
        for (size_t i = 0; i < sources.size() && i < GGML_MAX_SRC; i++) {
            ...
            if (t->type == GGML_TYPE_I32 || t->type == GGML_TYPE_I64) {
                if (op == GGML_OP_GET_ROWS || op == GGML_OP_GET_ROWS_BACK) {
                    ...
                    const int64_t nels = t->nelements();
...
public:
    ggml_tensor* llm_build_norm(
        ...
        llm_norm_type   type) {
        switch (type) {
        case LLM_NORM:     cur = ggml_norm(ctx, cur, hp.f_norm_eps, false); break;
        case LLM_NORM_RMS: cur = ggml_rms_norm(ctx, cur, hp.f_norm_rms_eps, false); break;
        }
        cur = ggml_mul(ctx, cur, mw, false);
        if (mb) {
            cur = ggml_add(ctx, cur, mb, false);
        }
        return cur;
    }
    ...
    ggml_tensor* llm_build_kqv(
        ...
        kq = ggml_soft_max(ctx, kq, false, kq_mask, kq_scale, 0.0f);
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        for (uint32_t il = 0; il < hp.n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                Qcur = ggml_rope_ext(
                    ctx, ggml_reshape(ctx, Qcur, { hp.n_embd_head, hp.n_head, hp.n_tokens }), inp_pos, nullptr,
                    hp.n_rot, 0, hp.n_ctx_orig, freq_base, freq_scale,
                    ext_factor, attn_factor, beta_fast, beta_slow, false
                );

                Kcur = ggml_rope_ext(
                    ctx, ggml_reshape(ctx, Kcur, { hp.n_embd_head, hp.n_head_kv, hp.n_tokens }), inp_pos, nullptr,
                    hp.n_rot, 0, hp.n_ctx_orig, freq_base, freq_scale,
                    ext_factor, attn_factor, beta_fast, beta_slow, false
                );
            ...
            cur = ggml_mul(ctx, cur, tmp, false);
            cur = ggml_mul_mat(ctx, ffn_down, cur);

            cur = ggml_add(ctx, cur, ffn_inp, false);
    ...
    ggml_tensor* build_graph(ggml_context* ctx) override {
        ...
        for (uint32_t il = 0; il < hp.n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                struct ggml_tensor* Qcur = ggml_cont(ctx, ggml_view(ctx, cur, { hp.n_embd, hp.n_tokens }, { cur->nb[1] }, 0 * sizeof(float) * (hp.n_embd)));
                struct ggml_tensor* Kcur = ggml_cont(ctx, ggml_view(ctx, cur, { hp.n_embd_gqa(), hp.n_tokens }, { cur->nb[1] }, 1 * sizeof(float) * (hp.n_embd)));
                struct ggml_tensor* Vcur = ggml_cont(ctx, ggml_view(ctx, cur, { hp.n_embd_gqa(), hp.n_tokens }, { cur->nb[1] }, 1 * sizeof(float) * (hp.n_embd + hp.n_embd_gqa())));

                Qcur = ggml_reshape(ctx, Qcur, { hp.n_embd_head, hp.n_head, hp.n_tokens });
                Kcur = ggml_reshape(ctx, Kcur, { hp.n_embd_head, hp.n_head_kv, hp.n_tokens });

                // using mode = 2 for neox mode
                Qcur = ggml_rope_ext(
                    ctx, Qcur, inp_pos, nullptr, hp.n_rot, 2, hp.n_ctx_orig,
                    freq_base, freq_scale, ext_factor, attn_factor, beta_fast, beta_slow, false
                );
        ...
        for (ggml_type type : all_types) {
            for (int b : {1, 7}) {
                for (bool v : {false, true}) {
                    ...
                    test_cases.emplace_back(new test_set_rows(src_type, type, GGML_TYPE_I64, { 3 * (int64_t)ggml_blck_size(type), 3, b, 1 }, { 2, 3, }, 2, v));
...
static bool run_fa_vec_slice(ggml_backend* backend, ggml_backend* backend_cpu, const char* op_names_filter) {
...
static bool test_backend(ggml_backend* backend, ggml_backend_device* dev, test_mode mode, const char* op_names_filter, const char* params_filter,
    ...
    if (mode == MODE_TEST) {
        ...
        const auto& run_tests = [&](ggml_backend* b, ggml_backend* b_cpu) {
        ...
        if (parallel_workers <= 1) {
            ...
            const auto& eval_worker = [&]() {
                std::unique_ptr<ggml_backend> b = dev->init_backend(NULL);
                if (b == NULL) {
                    return;
                }

                std::unique_ptr<ggml_backend> b_cpu = ggml_backend_init_by_type(GGML_BACKEND_DEVICE_TYPE_CPU, NULL);
                if (b_cpu == NULL) {
                    return;
                }

                backend_cpu->set_use_ref(true);
                workers_started++;
...
int main(int argc, char** argv) {
    ...
    output_printer->print_backend_status(
        backend_status_info(backend->get_name(), ok ? test_status_t::OK : test_status_t::FAIL));
}
