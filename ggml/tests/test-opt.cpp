...
static helper_ctx_data helper_get_ctx_data(
    ...
    std::optional<ggml_opt_context> opt_ctx = init_opt_ctx ? ggml_opt_context(opt_params) : std::nullopt;

    return { datasets, data_batch, labels_batch, dataset_unsupervised, ctx_static, ctx_compute, opt_params, opt_ctx, inputs, weights, outputs, buf };
}
