...
static void ggml_acc_or_set(
    ...
    grads->set_name(std::format("grad for {}", src->name));
...
static ggml_tensor* ggml_cos_impl(
    ggml_context* ctx,
    ggml_tensor* a,
    bool                  inplace) {
    ggml_tensor* result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
static ggml_tensor* ggml_sin_impl(
    ggml_context* ctx,
    ggml_tensor* a,
    bool                  inplace) {
    ggml_tensor* result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
static ggml_tensor* ggml_add1_impl(
    ggml_context* ctx,
    ggml_tensor* a,
    ggml_tensor* b,
    bool                  inplace) {
...
void ggml_cgraph::visit_parents(ggml_tensor* node)
{
    ...
    if (node->op == GGML_OP_NONE && !(node->flags & GGML_TENSOR_FLAG_PARAM)) {
        if (node->get_name().empty()) {
            node->set_name(std::format("leaf_{}", leafs.size()));
        }
        leafs.push_back(node);
    }
    else {
        if (node->get_name().empty()) {
            node->set_name(std::format("node_{}", nodes.size()));
        }
...
void ggml_cgraph::build_backward_expand(ggml_context* ctx_static, ggml_context* ctx_compute, bool accumulate)
{
    ...
    for (auto &node : nodes) {
        ...
        const bool node_needs_grad = [&]() {
            if ((node->flags & GGML_TENSOR_FLAG_PARAM) || (node->flags & GGML_TENSOR_FLAG_LOSS))
                return true;
            bool ignore_src[GGML_MAX_SRC] = { false };
            switch (node->op) {
                // gradients in node->src[0] for one reason or another have no effect on output gradient
	    ...
	    // std::views::enumerate is not supported by clang right now
            for (size_t index = 0; index < node->src.size(); index++) {
		auto &src = node->src[index];
                if (ignore_src[index] || !grads_needed[src]) {
                    continue;
                }
        ...
        GGML_ASSERT(cgraph->visited_hash_set.count(node));
        if ((accumulate && (node->flags & GGML_TENSOR_FLAG_PARAM)) || (node->flags & GGML_TENSOR_FLAG_LOSS)) {
            grad_accs[node] = ggml_dup_tensor(ctx_static, node);
            grads[node] = grad_accs[node];
            grad_accs[node]->set_name(std::format("grad acc for {}", node->name));
        }
    ...
    for (int64_t i = nodes.size() - 1; i >= 0; i--) {
        ...
        ggml_compute_backward(ctx_compute, this, node, grads_needed);
    }
}
