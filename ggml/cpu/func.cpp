...
static void ggml_compute_forward_conv_transpose_2d(
	...
	const int32_t padding0 = std::bit_cast<int32_t>(dst->op_params[1]);
	const int32_t padding1 = std::bit_cast<int32_t>(dst->op_params[2]);
					...
					stdexec::sender auto sender = stdexec::schedule(pool.get_scheduler()) | stdexec::then([=] {
						...
						for (int64_t cin = 0; cin < CIn; cin++) {
							for (int64_t kh = 0; kh < Kh; ++kh) {
								int64_t hin = hout - kh;
								if (hin < 0 || hin % stride) continue;
								hin /= stride;
								if (hin >= HIn) continue;

								for (int64_t kw = 0; kw < Kw; ++kw) {
									int64_t win = wout - kw;
									if (win < 0 || win % stride) continue;
									win /= stride;
									if (win >= WIn) continue;
