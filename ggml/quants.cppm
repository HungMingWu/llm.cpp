...
void quantize_row_q4_1_ref(const float* x, block_q4_1* y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        std::array<ggml_fp16_t, 2> dm{ fromFloat32<ggml_fp16_t>(d), fromFloat32<ggml_fp16_t>(min) };
        y[i].dm = std::bit_cast<uint32_t>(dm);
...
void quantize_row_q8_0_ref(const float* x, block_q8_0* y, int64_t k) {
    assert(k % block_q8_0::block_size == 0);
    const int nb = k / block_q8_0::block_size;

    for (int i = 0; i < nb; i++) {
        float amax = 0.0f; // absolute max

        for (int j = 0; j < block_q8_0::block_size; j++) {
            const float v = x[i * block_q8_0::block_size + j];
        ...
        for (int j = 0; j < block_q8_0::block_size; ++j) {
            const float x0 = x[i * block_q8_0::block_size + j] * id;
