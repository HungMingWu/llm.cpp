...
static constexpr size_t CUDA_IM2COL_BLOCK_SIZE = 256;
    ...
    int s0, int s1, int s2, int p0, int p1, int p2, int d0, int d1, int d2, cudaStream_t stream)
{
    if (dst_type == GGML_TYPE_F16) {
        im2col_3d_cuda(src1_d, (half*)dst_d, N, IC, ID, IH, IW, OC, KD, KH, KW, OD, OH, OW, s0, s1, s2, p0, p1, p2, d0, d1, d2, stream);
    }
    else {
        im2col_3d_cuda(src1_d, (float*)dst_d, N, IC, ID, IH, IW, OC, KD, KH, KW, OD, OH, OW, s0, s1, s2, p0, p1, p2, d0, d1, d2, stream);
    }
}