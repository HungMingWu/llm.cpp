...
static constexpr __device__ int fattn_tile_get_occupancy_device([[maybe_unused]] int ncols) {
#ifdef RDNA
    return 3;
#else
    return ncols <= 16 ? 3 : 2;
...
static constexpr __device__ int fattn_tile_get_kq_stride_device(int D, [[maybe_unused]] int ncols, [[maybe_unused]] int warp_size) {
#ifdef GGML_USE_HIP
#ifdef RDNA
    switch (D) {
    case 64:
...
static constexpr __device__ int fattn_tile_get_kq_nbatch_device(int D, [[maybe_unused]] int ncols, [[maybe_unused]] int warp_size) {
#ifdef GGML_USE_HIP
    switch (D) {
    case 64:
        return 64;
...
static void launch_fattn_tile_switch_ncols(const flash_attn_ext_context& ctx) {
    ...
    if (ctx.Q.ne1 > 16) {
        ...
        const int kq_stride = fattn_tile_get_kq_stride_host(D, cols_per_block, cc, warp_size);
        launch_fattn<D, cols_per_block, 1>
            (ctx, fattn_kernel, nwarps, nbytes_shared, kq_stride, true, true, false, warp_size);
        return;
    }
...
static void launch_fattn_tile_switch_head_size(const flash_attn_ext_context& ctx) {
    switch (ctx.Q.ne0) {
    case  64: {
        launch_fattn_tile_switch_ncols< 64, use_logit_softcap>(ctx);
    } break;
    case 128: {
        launch_fattn_tile_switch_ncols<128, use_logit_softcap>(ctx);
    } break;
    case 256: {
        launch_fattn_tile_switch_ncols<256, use_logit_softcap>(ctx);
    } break;
    default: {
...
void ggml_cuda_flash_attn_ext_tile(const flash_attn_ext_context& ctx)
{
    if (ctx.logit_softcap == 0.0f) {
        ...
        launch_fattn_tile_switch_head_size<use_logit_softcap>(ctx);
    }
    else {
