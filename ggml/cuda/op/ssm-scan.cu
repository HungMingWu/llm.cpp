...
void ssm_scan_ssd_f32_cuda(
    ...
    ggml_cuda_pool_alloc<float>    dt_sp_buf(pool, n_tok * n_head * n_seq);
    ggml_cuda_pool_alloc<float>    cs_buf(pool, n_tok * n_head * n_seq);
    ggml_cuda_pool_alloc<float>    CB_buf(pool, chunk_size * chunk_size * n_group * n_seq);
    ggml_cuda_pool_alloc<matmul_t> X_dt_buf(pool, chunk_size * head_dim * n_head * n_seq);
    ggml_cuda_pool_alloc<matmul_t> B_w_buf(pool, d_state * chunk_size * n_head * n_seq);
    ggml_cuda_pool_alloc<float>    C_s_buf(pool, d_state * chunk_size * n_head * n_seq);
    ...
    ggml_cuda_pool_alloc<half> M_buf(pool, n_M_max * n_head * n_seq);
