...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q8_0,  I);
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps) {
        ...
        const block_q1_0 * bxi = x + kbx0 + i*stride + scale_block;

        if constexpr (use_mma) {
            x_df[i*sram_stride                           + ksx] = std::bit_cast<half>(bxi->d);
        } else {
            x_df[i*(2*MMQ_TILE_NE_K/QI8_0) + i/(QI8_0/2) + ksx] = std::bit_cast<half>(bxi->d);
        }
    }
}

template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    const block_q2_0* __restrict__ x, int* __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    constexpr int nwarps = ggml_cuda_mmq_get_nthreads(type, J, fallback) / warp_size;
    constexpr int I = ggml_cuda_mmq_get_I(type, J, fallback);
    constexpr int sram_stride = ggml_cuda_mmq_get_sram_stride(type, J, fallback);
    ...
    constexpr int blocks_per_iter = MMQ_ITER_K / block_q2_0::block_size;
    constexpr int threads_per_row = blocks_per_iter * QI2_0;
    constexpr int nrows = warp_size / threads_per_row;
    constexpr int scale_entries_per_block = block_q2_0::block_size / block_q8_1::block_size;
    constexpr int scale_entries_per_row = blocks_per_iter * scale_entries_per_block;
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nrows*nwarps) {
        ...
        const block_q2_0 * bxi = x + kbx0 + i*stride + kbx;
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps) {
        ...
        const block_q2_0 * bxi = x + kbx0 + i*stride + scale_block;
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q4_0, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps * rows_per_warp) {
        ...
        const block_q4_0 * bxi = x + kbx0 + i*stride + kbxd;

        if constexpr (use_mma) {
            x_df[i*sram_stride                     + kbxd] = __half2float(std::bit_cast<half>(bxi->d));
        } else {
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    half2* x_dm = [=]() -> half2* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q4_1, I);
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps * rows_per_warp) {
        ...
        const block_q4_1 * bxi = x + kbx0 + i*stride + kbxd;

        if constexpr (use_mma) {
            x_dm[i*sram_stride                     + kbxd] = __tohalf2(bxi->dm);
        } else {
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q5_0, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps * rows_per_warp) {
        ...
        if constexpr (use_mma) {
            x_df[i*sram_stride                     + kbxd] = std::bit_cast<half>(bxi->d);
        } else {
            x_df[i*(MMQ_TILE_NE_K/QI5_0) + i/QI5_0 + kbxd] = std::bit_cast<half>(bxi->d);
        }
    }
}
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    half2* x_dm = [=]() -> half2* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q5_1, I);
            return (half2*)(x_qs + txs.qs);
        }
    }();
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps * rows_per_warp) {
        ...
        if constexpr (use_mma) {
            ...
            x_df[i*(2*MMQ_TILE_NE_K/QI8_0) + i/(QI8_0/2) + kbxd] = std::bit_cast<half>(bxi->d);
        }
    }
}
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    half2* x_dm = [=]() -> half2* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q2_K, I);
            return (half2*)(x_qs + txs.qs);
        }
    }();
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q3_K, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    half2* x_dm = [=]() -> half2* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q4_K, I);
            return (half2*)(x_qs + txs.qs);
        }
    }();
    int* x_sc = [=]() -> int* {
        ...
        for (int i0 = 0; i0 < I; i0 += nwarps * rows_per_warp) {
            auto [i, cond] = [=]() -> std::tuple<int, bool> {
                if constexpr (amd_mfma_available_v) {
                    ...
                    return { (i0 + threadIdx.y * rows_per_warp + threadIdx.x / 2) % I, true };
                }
            }();
            if (cond) {
                if (fallback) {
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    half2* x_dm = [=]() -> half2* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q5_K, I);
            return (half2*)(x_qs + txs.qs);
        }
    }();
    [[maybe_unused]] int* x_sc = [=]() -> int* {
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_Q6_K, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
    int* x_sc = [=]() -> int* {
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps*warp_size) {
        ...
        if constexpr (use_mma) {
            x_df[i * sram_stride]                     = __half2float(std::bit_cast<half>(bxi->d));
        } else {
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    half2* x_ds = [=]() -> half2* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_IQ3_S, I);
            return (half2*)(x_qs + txs.qs);
        }
    }();
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_IQ2_XXS, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
...
#pragma unroll
        ...
        const float d = std::bit_cast<half>(bxi->d);
        if constexpr (use_mma) {
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_IQ2_S, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_IQ3_XXS, I);
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_IQ4_XS, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nrows*nwarps) {
        ...
        const block_iq4_xs * bxi = x + kbx0 + i*stride;
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_IQ4_NL, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
        const block_mxfp4* __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride, std::true_type) {
    constexpr int warp_size   = ggml_cuda_get_physical_warp_size();
    constexpr int nwarps      = ggml_cuda_mmq_get_nthreads(type, J, fallback) / warp_size;
    constexpr int I           = ggml_cuda_mmq_get_I(type, J, fallback);
    constexpr int sram_stride = ggml_cuda_mmq_get_sram_stride(type, J, fallback);
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
    ...
    float* x_df = [=]() -> float* {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_MXFP4, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps * rows_per_warp) {
        ...
        const block_mxfp4 * bxi = x + kbx0 + i*stride + kbxd;
...
void mmq_load_tiles(const block_mxfp4* __restrict__ x, int* __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    if constexpr (blackwell_mma_available_v) {
        mmq_load_tiles<type, J, fallback>(x, x_tile, kbx0, i_max, stride, std::true_type{});
    }
    else {
        mmq_load_tiles<type, J, fallback>(x, x_tile, kbx0, i_max, stride, std::false_type{});
    }
}

template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
        const block_nvfp4* __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride, std::true_type) {
    constexpr int warp_size       = ggml_cuda_get_physical_warp_size();
    constexpr int nwarps          = ggml_cuda_mmq_get_nthreads(type, J, fallback) / warp_size;
    constexpr int I               = ggml_cuda_mmq_get_I(type, J, fallback);
    constexpr int iter_k          = ggml_cuda_mmq_get_K_vram(type, J, fallback);
    constexpr int threads_per_row = iter_k / block_nvfp4::block_size;
    constexpr int rows_per_warp   = warp_size / threads_per_row;
    constexpr int sram_stride     = ggml_cuda_mmq_get_sram_stride(type, J, fallback);
    ...
    const block_nvfp4 * bxi_base = x + kbx0 + kbx;
    uint32_t * x_u32_scale = x_u32 + 2 * MMQ_TILE_NE_K + kbx;
...
template <internal::ggml_type type, int J, bool fallback> static __device__ __forceinline__ void mmq_load_tiles(
        const block_nvfp4* __restrict__ x, int * __restrict__ x_tile, const int kb0, const int i_max, const int stride, std::false_type) {
    constexpr int warp_size   = ggml_cuda_get_physical_warp_size();
    constexpr int nwarps      = ggml_cuda_mmq_get_nthreads(type, J, fallback) / warp_size;
    constexpr int I           = ggml_cuda_mmq_get_I(type, J, fallback);
    constexpr int sram_stride = ggml_cuda_mmq_get_sram_stride(type, J, fallback);
    ...
    float* x_df = [=]() {
        if constexpr (use_mma) {
            ...
            constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(internal::GGML_TYPE_NVFP4, I);
            return (float*)(x_qs + txs.qs);
        }
    }();
...
template <internal::ggml_type type, int J, bool fallback>
static __device__ __forceinline__ void mmq_load_tiles(
        const block_nvfp4* __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    if constexpr (blackwell_mma_available_v) {
        mmq_load_tiles<type, J, fallback>(x, x_tile, kbx0, i_max, stride, std::true_type{});
    }
    else {
