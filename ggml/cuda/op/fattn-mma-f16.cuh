...
static constexpr __device__ fattn_mma_config ggml_cuda_fattn_mma_get_config(
    const int DKQ, const int DV, const int ncols) {
    if constexpr (ampere_mma_available_v)
        return ggml_cuda_fattn_mma_get_config_ampere(DKQ, DV, ncols);
...
static constexpr __device__ int get_cols_per_thread() {
    if constexpr (amd_wmma_available_v || amd_mfma_available_v)
...
static __device__ __forceinline__ void flash_attn_ext_f16_load_tile(
    ...
    const int D2 = KV.extent(1);
    const int chunks_per_row = D2 / h2_per_chunk;
    const int stride_tile = tile_KV.stride(0);
...
static __device__ __forceinline__ void flash_attn_ext_f16_load_mask(
    ...
    const int i_sup = mask_h.extent(1);
    if constexpr (use_cp_async) {
        ...
        const unsigned int tile_mask_32 = ggml_cuda_cvta_generic_to_shared(tile_mask.data_handle());
...
#pragma unroll
            for (int i0 = 0; i0 < nbatch_fa; i0 += 2*warp_size) {
                ...
                ggml_cuda_memcpy_1<sizeof(half2)>(
                    tile_mask.data_handle() + j_sram * stride_tile_mask + i,
                    mask_h.data_handle() + int64_t(j_vram) * stride_mask + i);
            }
        }
    }
...
{
    if constexpr (amd_wmma_available_v || amd_mfma_available_v) return 0;
    // Turing + Volta:
    return l % 2;
}

constexpr int calc_offset_first(int cols_per_warp)
{
    if constexpr (turing_mma_available_v) {
        // The partial sums are spread across 8/4 threads.
        return cols_per_warp == 8 ? 16 : 2;
    }
    else if constexpr (amd_mfma_available_v) {
...
constexpr int calc_offset_last(int cols_per_warp)
{
    if constexpr (turing_mma_available_v) {
        // The partial sums are spread across 8/4 threads.
        return cols_per_warp == 8 ? 4 : 1;
    }
    else if constexpr (amd_mfma_available_v) {
        // MFMA: 4 threads per Q column (threadIdx.x % 16 == col, spaced by 16).
        return 16;
...
static __device__ __forceinline__ void flash_attn_ext_f16_iter(
    [[maybe_unused]] const float2* const __restrict__ Q_f2,
    [[maybe_unused]] mdspan_stride_t<const half2, 2> K_h2,
    [[maybe_unused]] mdspan_stride_t<const half2, 2> V_h2,
    [[maybe_unused]] mdspan_stride_t<const half, 2> mask_h,
    [[maybe_unused]] float2* const __restrict__ dstk,
    [[maybe_unused]] float2* const __restrict__ dstk_fixup,
    [[maybe_unused]] const float scale,
    [[maybe_unused]] const float slope,
    [[maybe_unused]] const float logit_softcap,
    [[maybe_unused]] const uint3 ne01,
    [[maybe_unused]] const int ne02,
    [[maybe_unused]] half2* const __restrict__ tile_Q,
    [[maybe_unused]] half2* const __restrict__ tile_K,
    [[maybe_unused]] half2* const __restrict__ tile_V,
    [[maybe_unused]] half* const __restrict__ tile_mask,
    [[maybe_unused]] T_B_KQ* const __restrict__ Q_B,
    [[maybe_unused]] T_C_VKQ* const __restrict__ VKQ_C,
    [[maybe_unused]] float* const __restrict__ KQ_max,
    [[maybe_unused]] float* const __restrict__ KQ_rowsum,
    [[maybe_unused]] const int jt,
    [[maybe_unused]] const int kb0,
    [[maybe_unused]] const int k_VKQ_sup) {
    if constexpr (volta_mma_available_v || turing_mma_available_v || amd_wmma_available_v || amd_mfma_available_v) {
        constexpr int  warp_size       = ggml_cuda_get_physical_warp_size();
        ...
        std::mdspan tile_K_mdspan(tile_K, nbatch_fa, stride_tile_K);
        std::mdspan tile_V_mdspan(tile_V, nbatch_fa, stride_tile_V);
        std::mdspan tile_mask_mdspan(tile_mask, ncols1, nbatch_fa + 8);

        const int k_VKQ_0 = kb0 * nbatch_fa;
        constexpr size_t factor = [=]() {
            if constexpr (turing_mma_available_v)
                return cols_per_warp == 8 ? T_C_KQ::I : T_C_KQ::J;
            else if constexpr (amd_wmma_available_v || amd_mfma_available_v)
                return T_C_KQ::J;
            else
                return T_C_KQ::J;
        }();
        T_C_KQ KQ_C[nbatch_fa / (np * factor)];
        ...
        if constexpr (nstages > 1) {
            ...
            flash_attn_ext_f16_load_tile<nwarps, nbatch_fa, use_cp_async, oob_check>(
                std::submdspan(V_h2, std::pair{ k_VKQ_0, k_VKQ_0 + k_VKQ_sup }, std::pair{ 0, nbatch_V2 }),
                std::submdspan(tile_V_mdspan, std::full_extent, std::pair{ 0, nbatch_V2 }));
        }
        else {
            constexpr bool use_cp_async = nstages == 1;
            if (ncols2 > 1 || mask_h.data_handle()) {
                flash_attn_ext_f16_load_mask<ncols1, nwarps, nbatch_fa, use_cp_async, oob_check>
                    (std::submdspan(mask_h, std::full_extent, std::pair{ k_VKQ_0, k_VKQ_0 + k_VKQ_sup }),
                     std::submdspan(tile_mask_mdspan, std::full_extent, std::pair{ 0, nbatch_fa }),
                     jt * ncols1, ne01);
            }
        }
...
#pragma unroll
        for (int k0_start = (DKQ / 2 - 1) - (DKQ / 2 - 1) % nbatch_K2; k0_start >= 0; k0_start -= nbatch_K2) {
            ...
            if constexpr (nstages <= 1) {
                ...
                constexpr bool use_cp_async = nstages == 1;
                flash_attn_ext_f16_load_tile<nwarps, nbatch_fa, use_cp_async, oob_check>(
                    std::submdspan(K_h2, std::pair{ k_VKQ_0, k_VKQ_0 + k_VKQ_sup }, std::pair{ k0_start, k0_stop }),
                    std::submdspan(tile_K_mdspan, std::full_extent, std::pair{ 0, k0_diff }));
                if (use_cp_async) {
                    cp_async_wait_all();
                }
                __syncthreads();
...
#pragma unroll
                for (int l = 0; l < T_C_KQ::ne; ++l) {
                    if (!oob_check || k0 + (threadIdx.y % np) * T_C_KQ::I + T_C_KQ::get_i(l) < k_VKQ_sup) {
                        const int KQ_idx = calc_KQ_idx(l);
...
#pragma unroll
                for (int l = 0; l < T_C_KQ::ne; ++l) {
                    if (!oob_check || k0 + (threadIdx.y % np) * T_C_KQ::J + T_C_KQ::get_j(l) < k_VKQ_sup) {
                        const int KQ_idx = calc_KQ_idx(l / 2);
...
#pragma unroll
            for (int col = 0; col < cols_per_thread; ++col) {
                constexpr int offset_first = calc_offset_first(cols_per_warp);
                constexpr int offset_last = calc_offset_last(cols_per_warp);
#pragma unroll
...
#pragma unroll
        ...
        if constexpr (nstages > 1) {
            ...
            if (!last_iter) {
                ...
                if (ncols2 > 1 || mask_h.data_handle()) {
                    flash_attn_ext_f16_load_mask<ncols1, nwarps, nbatch_fa, use_cp_async, oob_check>
                        (std::submdspan(mask_h, std::full_extent, std::pair{ k_VKQ_0 + nbatch_fa, k_VKQ_0 + nbatch_fa + k_VKQ_sup }),
                         std::submdspan(tile_mask_mdspan, std::full_extent, std::pair{ 0, nbatch_fa }),
                         jt * ncols1, ne01);
                }
                flash_attn_ext_f16_load_tile<nwarps, nbatch_fa, use_cp_async, oob_check>(
                    std::submdspan(K_h2, std::pair{ k_VKQ_0 + nbatch_fa, k_VKQ_0 + 2 * nbatch_fa }, std::pair{ 0, nbatch_K2 }),
                    std::submdspan(tile_K_mdspan, std::full_extent, std::pair{ 0, nbatch_K2 }));
            }
        }
        ...
        for (int i0_start = 0; i0_start < DV; i0_start += 2 * nbatch_V2) {
            ...
            if constexpr (nstages <= 1) {
                ...
                if (!V_is_K_view || i0_stop > 2 * nbatch_K2) {
                    constexpr bool use_cp_async = nstages == 1;
                    flash_attn_ext_f16_load_tile<nwarps, nbatch_fa, use_cp_async, oob_check>(
                        std::submdspan(V_h2, std::pair{ k_VKQ_0, k_VKQ_0 + k_VKQ_sup }, std::pair{ i0_start / 2, i0_stop / 2 }),
                        std::submdspan(tile_V_mdspan, std::full_extent, std::pair{ 0, i0_diff / 2 }));
                    if (use_cp_async) {
                        cp_async_wait_all();
                    }
                    __syncthreads();
            ...
            if constexpr (turing_mma_available_v || amd_wmma_available_v || amd_mfma_available_v) {
#pragma unroll
...
static constexpr bool RDNA3_v = false;

template <int DV, int ncols>
constexpr auto get_T_A_KQ_type()
{
    if constexpr (turing_mma_available_v) {
        return tile<16, 8, half2>{}; // row-major
    }
    else if constexpr (amd_wmma_available_v) {
        if constexpr (RDNA3_v) {
            return tile<16,  8, half2, DATA_LAYOUT_I_MAJOR_MIRRORED>{}; // row-major
        } else {
            if constexpr (DV == 80 || DV == 112)
                return tile<16,  8, half2>{}; // row-major
            else
                return tile<16, 8, half2>{}; // row-major
        }
        ...
        return tile<8,  4, half2, DATA_LAYOUT_I_MAJOR_MIRRORED>{}; // row-major
    }
}

template <int DV, int ncols>
constexpr auto get_T_B_KQ_type()
{
    if constexpr (turing_mma_available_v) {
        if constexpr (ncols != 8)
            return tile<16, 8, half2>{}; // column-major
        else 
            return tile<8, 8, half2>{}; // column-major
    }
    else if constexpr (amd_wmma_available_v) {
        if constexpr (RDNA3_v) {
            return tile<16, 8, half2, DATA_LAYOUT_I_MAJOR_MIRRORED>{}; // column-major
        } else {
            if constexpr (DV == 80 || DV == 112)
                return tile<16,  8, half2>{}; // column-major
            else
                return tile<16,  8, half2, DATA_LAYOUT_I_MAJOR>{};           // row-major
        }
    }
    else if constexpr (amd_mfma_available_v) {
        return tile<16,  8, half2>{}; // column-major
        ...
        return tile<32, 4, half2, DATA_LAYOUT_I_MAJOR>{}; // column-major
    }
}

template <int DV, int ncols>
constexpr auto get_T_C_KQ_type()
{
    if constexpr (turing_mma_available_v) {
        if constexpr (ncols != 8)
            return tile<16, 16, float>{}; // column-major
        else
            return tile<16, 8, float>{}; // row-major
    }
    else if constexpr (amd_wmma_available_v) {
        if constexpr (RDNA3_v) {
            return tile<16, 16, float, DATA_LAYOUT_I_MAJOR>{}; // column-major
        } else {
            if constexpr (DV == 80 || DV == 112)
                return tile<16, 16, float>{}; // column-major
            else
        ...
        return tile<32,  8, float, DATA_LAYOUT_I_MAJOR>{}; // column-major
    }
}

template <int DV, int ncols>
constexpr auto get_T_A_VKQ_type()
{
    if constexpr (turing_mma_available_v) {
        return tile<16, 8, half2>{}; // row-major
    }
        ...
        if constexpr (RDNA3_v) {
            if constexpr (DV == 80 || DV == 112)
                ...
                return tile<32,  8, half2, DATA_LAYOUT_I_MAJOR_MIRRORED>{}; // row-major
        } else {
            if constexpr (DV == 80 || DV == 112)
                return tile<16,  8, half2>{}; // row-major
            else
                return tile<32,  8, half2, DATA_LAYOUT_I_MAJOR>{}; // row-major
        }
    }
    else if constexpr (amd_mfma_available_v) {
        return tile<16,  8, half2>{}; // row-major
        ...
        return tile< 8,  4, half2, DATA_LAYOUT_J_MAJOR_MIRRORED>{}; // column-major
    }
}

template <int DV, int ncols>
constexpr auto get_T_B_VKQ_type()
{
    if constexpr (turing_mma_available_v) {
        if constexpr (ncols != 8)
            return tile<16, 8, half2>{}; // column-major
        ...
        if constexpr (RDNA3_v) {
            ...
            if constexpr (DV == 80 || DV == 112)
                ...
                return tile<16,  8, half2, DATA_LAYOUT_I_MAJOR>{}; // column-major
        }
    }
    else if constexpr (amd_mfma_available_v) {
        return tile<16,  8, half2>{}; // column-major
...
template <int DV, int ncols>
constexpr auto get_T_C_VKQ_type()
{
    if constexpr (turing_mma_available_v) {
        if constexpr (ncols != 8)
            return tile<16, 8, half2>{}; // column-major
        else 
            return tile<16, 4, half2>{}; // row-major
    }
    else if constexpr (amd_wmma_available_v) {
        if constexpr (RDNA3_v) {
            if constexpr (DV == 80 || DV == 112)
                ...
                return tile<16, 16, half2, DATA_LAYOUT_I_MAJOR>{}; // column-major
        } else {
            if constexpr (DV == 80 || DV == 112)
                return tile<16,  8, half2>{}; // column-major
            else
                return tile<16, 16, half2, DATA_LAYOUT_I_MAJOR_SCRAMBLED>{}; // column-major
        }
    }
    else if constexpr (amd_mfma_available_v) {
        return tile<16,  8, half2>{}; // column-major
...
static __device__ __forceinline__ void flash_attn_ext_f16_process_tile(
    ...
    [[maybe_unused]] mdspan_stride_t<const half2, 2> V_h2,
    [[maybe_unused]] mdspan_stride_t<const half, 2> mask_h,
    [[maybe_unused]] const float* const __restrict__ sinks_f,
    [[maybe_unused]] float2* const __restrict__ dstk,
    [[maybe_unused]] float2* const __restrict__ dstk_fixup,
    [[maybe_unused]] const float scale,
    [[maybe_unused]] const float slope,
    ...
    [[maybe_unused]] const uint3 ne01,
    [[maybe_unused]] const int ne02,
    [[maybe_unused]] const int gqa_ratio,
    [[maybe_unused]] const int ne11,
    [[maybe_unused]] const int stride_Q1,
    [[maybe_unused]] const int stride_Q2,
    [[maybe_unused]] const int jt,
    [[maybe_unused]] const int zt_gqa,
    [[maybe_unused]] const int kb0_start,
    [[maybe_unused]] const int kb0_stop) {
    if constexpr (volta_mma_available_v || turing_mma_available_v || amd_wmma_available_v || amd_mfma_available_v) {
        //In this kernel Q, K, V are matrices while i, j, k are matrix indices.

        constexpr int warp_size = ggml_cuda_get_physical_warp_size();
        constexpr int ncols = ncols1 * ncols2;
        using     T_A_KQ = decltype(get_T_A_KQ_type<DV, ncols>());
        using     T_B_KQ = decltype(get_T_B_KQ_type<DV, ncols>());
        using     T_C_KQ = decltype(get_T_C_KQ_type<DV, ncols>());
        using     T_A_VKQ = decltype(get_T_A_VKQ_type<DV, ncols>());
        using     T_B_VKQ = decltype(get_T_B_VKQ_type<DV, ncols>());
        using     T_C_VKQ = decltype(get_T_C_VKQ_type<DV, ncols>());
        ...
        static constexpr size_t VKQ_C_elements = [=]() {
            if (turing_mma_available_v)
                return cols_per_warp == 8 ? DV / T_C_VKQ::I : DV / (2 * T_C_VKQ::J);
            else if (amd_wmma_available_v) {
#if defined(RDNA3)
                return DV % 32 != 0       ? DV/T_C_VKQ::J : DV/(2*T_C_VKQ::J);
#else
                return DV / (2 * T_C_VKQ::J);
#endif
            }
            else
                return DV / (2 * T_C_VKQ::J);
        }();
        T_C_VKQ VKQ_C[VKQ_C_elements];
...
#pragma unroll
        ...
        if constexpr (nstages > 1) {
            ...
            if (ncols2 > 1 || mask_h.data_handle()) {
                flash_attn_ext_f16_load_mask<ncols1, nwarps, nbatch_fa, use_cp_async, oob_check>
                    (std::submdspan(mask_h, std::full_extent, std::pair{ kb0 * nbatch_fa, kb0 * nbatch_fa + k_VKQ_sup }),
                     std::submdspan(tile_mask_mdspan, std::full_extent, std::pair{ 0, nbatch_fa }),
                     jt * ncols1, ne01);
            }
            flash_attn_ext_f16_load_tile<nwarps, nbatch_fa, use_cp_async, oob_check>(
                std::submdspan(K_h2, std::pair{ kb0 * nbatch_fa, (kb0 + 1) * nbatch_fa }, std::pair{ 0, nbatch_K2 }),
                std::submdspan(tile_K_mdspan, std::full_extent, std::pair{ 0, nbatch_K2 }));
        }
            ...
            for (; kb0 < kb0_stop - 1; ++kb0) {
                ...
                flash_attn_ext_f16_iter
                    ...
                    (Q_f2, K_h2, V_h2, mask_h, dstk, dstk_fixup, scale, slope, logit_softcap,
                        ne01, ne02, tile_Q, tile_K, tile_V, tile_mask, Q_B, VKQ_C,
...
#pragma unroll
        ...
        if constexpr (cols_per_warp == 8) {
            ...
            const int jc_cwm = [=]() {
                if constexpr (turing_mma_available_v)
                    return threadIdx.y * cols_per_warp + T_C_VKQ::get_i(threadIdx.x % 4);
                else if constexpr (amd_wmma_available_v || amd_mfma_available_v)
                    return threadIdx.y * cols_per_warp + T_C_VKQ::get_i(0);
                else // Volta
                    return threadIdx.y*cols_per_warp + T_C_KQ::get_i(threadIdx.x & 2);
            }();
            const float2 KQ_cmr = [=]() {
                if constexpr (turing_mma_available_v)
                    return make_float2(KQ_max[threadIdx.x % cols_per_thread], KQ_rowsum[threadIdx.x % cols_per_thread]);
                else if constexpr (amd_wmma_available_v || amd_mfma_available_v)
                    return make_float2(KQ_max[0], KQ_rowsum[0]);
                else // Volta
                    return make_float2(KQ_max[(threadIdx.x & 2) / 2], KQ_rowsum[(threadIdx.x & 2) / 2]);
            }();
            const bool thread_should_write = [=]() {
                if constexpr (turing_mma_available_v)
                    return threadIdx.x % 4 < cols_per_thread;
                else if constexpr (amd_wmma_available_v || amd_mfma_available_v)
                    return threadIdx.x / 16 < cols_per_thread;
                else // Volta
                    return T_C_KQ::J == 8 || T_C_KQ::get_j(threadIdx.x & 2) < 8;
            }();
...
static __global__ void flash_attn_ext_f16(
    ...
    if constexpr (flash_attn_available_v && (volta_mma_available_v || turing_mma_available_v || amd_wmma_available_v || amd_mfma_available_v)) {
        ...
        std::array<int64_t, 4> K_ne = { ne10 / 2, ne11, ne12, ne13 };
        std::array<size_t, 4> K_nb = { sizeof(half2), size_t(nb11), size_t(nb12), size_t(nb13) };
        auto K_data_4d = make_strided_mdspan(reinterpret_cast<const half2*>(K), K_ne, K_nb);
        std::array<int64_t, 4> V_ne = { ne10 / 2, ne11, ne12, ne13 };
        std::array<size_t, 4> V_nb = { sizeof(half2), size_t(nb21), size_t(nb22), size_t(nb23) };
        auto V_data_4d = make_strided_mdspan(reinterpret_cast<const half2*>(V), V_ne, V_nb);
        auto mask_data_4d = [=]() -> mdspan_stride_t<const half, 4> {
            if (!ctx.mask.data) {
                return {};
            }
            return make_strided_mdspan((const half*)(ctx.mask.data), ctx.mask.ne, ctx.mask.nb);
        }();
        ...
        while (kbc < kbc_stop && kb0_stop == iter_k) {
            ...
            mdspan_stride_t<const half2, 2> K_h2 = std::submdspan(K_data_4d, sequence, z_KV, std::full_extent, std::full_extent);
            auto mask_h = [=]() -> mdspan_stride_t<const half, 2> {
                if (ncols2 == 1 && !ctx.mask.data) {
                    return {};
                }
                return std::submdspan(mask_data_4d, sequence % ctx.mask.ne[3], 0, std::full_extent, std::full_extent);
            }();
            float2* dstk = ((float2*)dst) + (sequence * ne01.z * ne02 + zt_Q) * (DV / 2);

            auto V_h2 = [=]() -> mdspan_stride_t<const half2, 2> {
                if constexpr (V_is_K_view) {
                    return K_h2;
                }
                return std::submdspan(V_data_4d, sequence, z_KV, std::full_extent, std::full_extent);
            }();
            const char* __restrict__ sinks = (const char*)ctx.sinks.data;
            const float* sinks_f = sinks ? (const float*)sinks + zt_Q : nullptr;

            const float slope = ncols2 == 1 ? get_alibi_slope(ctx.max_bias, zt_Q, n_head_log2, m0, m1) : 1.0f;
            ...
            if (kb0_start == 0) {
                ...
                flash_attn_ext_f16_process_tile<DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, needs_fixup, is_fixup>
                    (Q_f2, K_h2, V_h2, mask_h, sinks_f, dstk, dst_meta, scale, slope, ctx.logit_softcap,
                        ne01, ne02, gqa_ratio, ne11, stride_Q1, stride_Q2, jt, zt_gqa, kb0_start, kb0_stop);
            }
            else {
...
void ggml_cuda_flash_attn_ext_mma_f16_case(const flash_attn_ext_context& ctx) {
    ...
    launch_fattn<DV, ncols1, ncols2>
        (ctx, fattn_kernel, nwarps, nbytes_shared_total, nbatch_fa, true, true, true, warp_size_host);
}
