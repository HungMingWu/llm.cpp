...
#pragma unroll
        ...
        if constexpr (cols_per_warp == 8) {
            ...
            const int jc_cwm = (turing_mma_available_v) ?
                threadIdx.y * cols_per_warp + T_C_VKQ::get_i(threadIdx.x % 4) :
                threadIdx.y * cols_per_warp + T_C_KQ::get_i(threadIdx.x & 2);
            const float2 KQ_cmr = (turing_mma_available_v) ?
                make_float2(KQ_max[threadIdx.x % cols_per_thread], KQ_rowsum[threadIdx.x % cols_per_thread]) :
                make_float2(KQ_max[(threadIdx.x & 2) / 2], KQ_rowsum[(threadIdx.x & 2) / 2]);
            const bool thread_should_write = (turing_mma_available_v) ?
                (threadIdx.x % 4 < cols_per_thread) : (T_C_KQ::J == 8 || T_C_KQ::get_j(threadIdx.x & 2) < 8);
...
static __global__ void flash_attn_ext_f16(
    ...
    if constexpr (flash_attn_available_v && (volta_mma_available_v || turing_mma_available_v || amd_wmma_available_v)) {
...
void ggml_cuda_flash_attn_ext_mma_f16_case(const flash_attn_ext_context& ctx) {
    ...
    launch_fattn<DV, ncols1, ncols2>
        (ctx, fattn_kernel, nwarps, nbytes_shared_total, nbatch_fa, true, true, true);
}
