...
static __device__ __forceinline__ void dequantize(const block_q1_0 *x, const int iqs, float2 & v){
    ...
    const int bit_0 = (x->qs[byte_index_0] >> bit_offset_0) & 1;
    const int bit_1 = (x->qs[byte_index_1] >> bit_offset_1) & 1;

    v.x = (2*bit_0 - 1) * d;
    v.y = (2*bit_1 - 1) * d;
}

static __device__ __forceinline__ void dequantize(const block_q2_0 * x, const int iqs, float2 & v){
    const float d = std::bit_cast<half>(x->d);
    ...
    const int c0 = (x->qs[byte_index_0] >> bit_offset_0) & 0x3;
    const int c1 = (x->qs[byte_index_1] >> bit_offset_1) & 0x3;
...
static __device__ __forceinline__ void dequantize(const block_q2_K * x, const int64_t ib, dst_t * yy, const int tid) {
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_q3_K * x, const int64_t ib, dst_t * yy, const int tid) {
    ...
    float d_all = std::bit_cast<half>(x[ib].d);
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_q4_K * x, const int64_t ib, dst_t * yy, const int tid) {
    // assume 32 threads
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_q5_K * x, const int64_t ib, dst_t * yy, const int tid) {
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_q6_K * x, const int64_t ib, dst_t * yy, const int tid) {
    // assume 64 threads - this is very slightly better than the one below
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_iq2_xxs * x, const int64_t ibs, dst_t * yy, const int tid) {
    ...
    const float d = (float)std::bit_cast<half>(x[ibs].d) * (0.5f + (aux32 >> 28)) * 0.25f;
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_iq2_xs * x, const int64_t ibs, dst_t * yy, const int tid) {
    const int64_t il = tid/8; // 0...3
    const int64_t ib = tid%8; // 0...7
    dst_t * y = yy + 32*ib + 8*il;
    const uint16_t * q2 = x[ibs].qs + 4*ib;
    const uint8_t  * grid = (const uint8_t *)(iq2xs_grid + (q2[il] & 511));
    const float d = (float)std::bit_cast<half>(x[ibs].d) * (0.5f + ((x[ibs].scales[ib] >> 4*(il/2)) & 0xf)) * 0.25f;
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_iq2_s * x, const int64_t ibs, dst_t * yy, const int tid) {
    const int64_t il = tid/8; // 0...3
    const int64_t ib = tid%8; // 0...7
    dst_t * y = yy + 32*ib + 8*il;
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_iq3_xxs * x, const int64_t ibs, dst_t * yy, const int tid) {
    const int64_t il = tid/8; // 0...3
    const int64_t ib = tid%8; // 0...7
    dst_t * y = yy + 32*ib + 8*il;
    ...
    const float d = (float)std::bit_cast<half>(x[ibs].d) * (0.5f + (aux32 >> 28)) * 0.5f;
    const uint8_t signs = ksigns_iq2xs[(aux32 >> 7*il) & 127];
    for (int j = 0; j < 4; ++j) {
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_iq3_s * x, const int64_t ibs, dst_t * yy, const int tid) {
    const int64_t il = tid/8; // 0...3
    const int64_t ib = tid%8; // 0...7
    dst_t * y = yy + 32*ib + 8*il;
    ...
    const float d = (float)std::bit_cast<half>(x[ibs].d) * (1 + 2*((x[ibs].scales[ib/2] >> 4*(ib%2)) & 0xf));
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_iq1_s * x, const int64_t ibs, dst_t * yy, const int tid) {
    const int64_t il = tid/8; // 0...3
    const int64_t ib = tid%8; // 0...7
    dst_t * y = yy + 32*ib + 8*il;
    const float delta = x[ibs].qh[ib] & 0x8000 ? -1 - IQ1S_DELTA : -1 + IQ1S_DELTA;
    const float d = (float)std::bit_cast<half>(x[ibs].d) * (2*((x[ibs].qh[ib] >> 12) & 7) + 1);
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_iq1_m * x, const int64_t ibs, dst_t * yy, const int tid) {
    const int64_t il = tid/8; // 0...3
    const int64_t ib = tid%8; // 0...7
    dst_t * y = yy + 32*ib + 8*il;
    ...
    const float d = (float)std::bit_cast<half>(scale) * (2*((sc[ib16/4] >> 3*(ib16%4)) & 0x7) + 1);
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_iq4_nl * vx, const int64_t ibs, dst_t * yy, const int tid) {

    const block_iq4_nl * x = (const block_iq4_nl *) vx + ibs*(QK_K/block_iq4_nl::block_size);

    const int64_t il = tid/8; // 0...3
    const int64_t ib = tid%8; // 0...7
    dst_t * y = yy + 32*ib + 4*il;
    const uint8_t  * q4 = x[ib].qs + 4*il;
    const float d = (float)std::bit_cast<half>(x[ib].d);
    for (int j = 0; j < 4; ++j) {
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_iq4_xs * x, const int64_t ibs, dst_t * yy, const int tid) {
    const int64_t il = tid/8; // 0...3
    const int64_t ib = tid%8; // 0...7
    dst_t * y = yy + 32*ib + 4*il;
    const uint8_t  * q4 = x[ibs].qs + 16*ib + 4*il;
    const float d = (float)std::bit_cast<half>(x[ibs].d) * ((((x[ibs].scales_l[ib/2] >> 4*(ib%2)) & 0xf) | (((x[ibs].scales_h >> 2*ib) & 3) << 4)) - 32);
    for (int j = 0; j < 4; ++j) {
        y[j+ 0] = ggml_cuda_cast<dst_t>(d * kvalues_iq4nl[q4[j] & 0xf]);
        y[j+16] = ggml_cuda_cast<dst_t>(d * kvalues_iq4nl[q4[j] >>  4]);
    }
...
template <typename dst_t>
static __device__ __forceinline__ void dequantize(const block_mxfp4 * vx, const int64_t ibs, dst_t * yy, const int tid) {
    const block_mxfp4 * x = vx + ibs*(QK_K/block_mxfp4::block_size);
