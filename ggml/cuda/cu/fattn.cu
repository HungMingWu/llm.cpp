...
void ggml_cuda_flash_attn_ext_vec_f16_case_impl(const flash_attn_ext_context& ctx) {
    ...
    launch_fattn<D, cols_per_block, 1, parallel_blocks, -1>(ctx, fattn_kernel, nwarps, nbytes_shared, need_f16_K, need_f16_V);
}
...
void ggml_cuda_flash_attn_ext_vec_f16_case(const flash_attn_ext_context& ctx) {
    ...
    if (ctx.Q.ne1 == 1) {
        ...
        if (ctx.logit_softcap == 0.0f) {
            ...
            ggml_cuda_flash_attn_ext_vec_f16_case_impl<D, cols_per_block, parallel_blocks, type_K, type_V, use_logit_softcap>(ctx);
        }
        else {
...
void ggml_cuda_flash_attn_ext_vec_f32_case(const flash_attn_ext_context& ctx) {
    ...
    if (ctx.Q.ne1 == 1) {
        ...
        if (ctx.logit_softcap == 0.0f) {
            constexpr bool use_logit_softcap = false;
            ggml_cuda_flash_attn_ext_vec_f32_case_impl<D, cols_per_block, parallel_blocks, type_K, type_V, use_logit_softcap>(ctx);
        }
        else {
            constexpr bool use_logit_softcap = true;
            ggml_cuda_flash_attn_ext_vec_f32_case_impl<D, cols_per_block, parallel_blocks, type_K, type_V, use_logit_softcap>(ctx);
...
void launch_fattn_tile_f32_64_128(const flash_attn_ext_context& ctx) {
    ...
    case  64: {
        ...
        launch_fattn<D, cols_per_block, 1, parallel_blocks, -1>(ctx, fattn_kernel, nwarps, nbytes_shared, true, true);
    } break;
    case 128: {
        constexpr int    D = 128;
        constexpr int    nwarps = 8;
...
void ggml_cuda_flash_attn_ext_tile_f32(const flash_attn_ext_context& ctx) {
    if (ctx.Q.ne1 <= 16) {
        ...
        if (ctx.logit_softcap == 0.0f) {
            constexpr bool use_logit_softcap = false;
            launch_fattn_tile_f32_64_128<cols_per_block, parallel_blocks, use_logit_softcap>(ctx);
        }
        else {
            constexpr bool use_logit_softcap = true;
            launch_fattn_tile_f32_64_128<cols_per_block, parallel_blocks, use_logit_softcap>(ctx);
