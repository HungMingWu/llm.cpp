...
template <size_t vdr>
static __device__ __forceinline__  int calc_sum(const int(&v)[vdr], const int(&u)[2 * vdr])
{
    int sumi = 0;
#pragma unroll
    for (int i = 0; i < vdr; ++i) {
...
static constexpr size_t VDR_Q4_0_Q8_1_MMVQ = 2;

template <size_t vdr> static __device__ __forceinline__ float vec_dot_q4_0_q8_1_impl(
    const int(&v)[vdr], const int(&u)[2 * vdr], const float& d4, const half2& ds8) {

    const int sumi = calc_sum(v, u);
...
template <size_t vdr> static __device__ __forceinline__ float vec_dot_q4_1_q8_1_impl(
    const int(&v)[vdr], const int(&u)[2 * vdr], const half2& dm4, const half2& ds8) {
...
static constexpr size_t VDR_Q4_1_Q8_1_MMVQ = 2;
