...
void launch_fattn_tile_f16_64_128(const flash_attn_ext_context& ctx) {
    switch (ctx.Q.ne0) {
    case  64: {
        constexpr int    D = 64;
        ...
        launch_fattn<D, cols_per_block, 1>
            (ctx, fattn_kernel, nwarps, nbytes_shared, FATTN_KQ_STRIDE_TILE_F16, true, true, false);
    } break;
    case 128: {
        constexpr int    D = 128;
        constexpr int    nwarps = 8;
...
void ggml_cuda_flash_attn_ext_tile_f16(const flash_attn_ext_context& ctx) {
    if (ctx.Q.ne1 <= 16) {
        ...
        if (ctx.logit_softcap == 0.0f) {
            ...
            launch_fattn_tile_f16_64_128<cols_per_block, use_logit_softcap>(ctx);
        }
        else {
