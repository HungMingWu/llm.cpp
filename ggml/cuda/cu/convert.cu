...
static __global__ void convert_unary(const src_t* __restrict__ x, dst_t* __restrict__ y, const int64_t k) {
...
template <typename src_t, typename dst_t>
static void convert_unary_cuda(const src_t* __restrict__ x, dst_t* __restrict__ y, const int64_t k, cudaStream_t stream) {
    const int num_blocks = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    convert_unary<src_t> << <num_blocks, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream >> > (x, y, k);
}
...
void to_fp32_cuda(ggml_type type, const void* x, float* y, int64_t k, cudaStream_t stream)
{
    ...
    case GGML_TYPE_Q5_0:
        return dequantize_block_cuda<block_q5_0::block_size, QR5_0, dequantize_q5_0>(x, y, k, stream);
    case GGML_TYPE_Q5_1:
        return dequantize_block_cuda<block_q5_1::block_size, QR5_1, dequantize_q5_1>(x, y, k, stream);
    case GGML_TYPE_Q8_0:
        return dequantize_block_cuda<block_q8_0::block_size, QR8_0, dequantize_q8_0>(x, y, k, stream);
    case GGML_TYPE_Q2_K:
    ...
    case GGML_TYPE_F16:
        return convert_unary_cuda(static_cast<const half*>(x), y, k, stream);
    case GGML_TYPE_BF16:
        return convert_unary_cuda(static_cast<const nv_bfloat16*>(x), y, k, stream);
    default:
        assert(false);
    }
}
...
void to_fp16_cuda(ggml_type type, const void* x, half* y, int64_t k, cudaStream_t stream)
{
    ...
    case GGML_TYPE_IQ3_S:
        return dequantize_row_cuda(static_cast<const block_iq3_s*>(x), y, k, stream);
    case GGML_TYPE_F32:
        return convert_unary_cuda(static_cast<const float*>(x), y, k, stream);
    default:
