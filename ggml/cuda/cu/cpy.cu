...
static __device__ void copy_block(const block_q8_0* xi, float* dsti) {
...
concept simplest_case_v =
    (std::is_same_v<src_t, float> && (std::is_same_v<dst_t, float> || std::is_same_v<dst_t, half>)) ||
...
static __global__ void cpy(dup_context ctx) {
    ...
    const int64_t x_offset = [=] {
        ...
        else if constexpr (std::is_same_v<src_t, block_q8_0> && std::is_same_v<dst_t, float>) {
...
static void ggml_cpy_cuda(const dup_context &ctx, cudaStream_t stream) {
    ...
    if constexpr (simplest_case_v<src_t, dst_t>) {
        ...
        cpy<block_q8_0, float> << <num_blocks, 1, 0, stream >> > (ctx);
    }
}
