...
void sqr_f32_cuda(const float* x, float* dst, const int k, cudaStream_t stream) {
    static constexpr size_t CUDA_SQR_BLOCK_SIZE = 256;
    const int num_blocks = (k + CUDA_SQR_BLOCK_SIZE - 1) / CUDA_SQR_BLOCK_SIZE;
    transform <sqr> << <num_blocks, CUDA_SQR_BLOCK_SIZE, 0, stream >> > (x, dst, k);
}

void sqrt_f32_cuda(const float* x, float* dst, const int k, cudaStream_t stream) {
    static constexpr size_t CUDA_SQRT_BLOCK_SIZE = 256;
    const int num_blocks = (k + CUDA_SQRT_BLOCK_SIZE - 1) / CUDA_SQRT_BLOCK_SIZE;
    transform <sqrtf> << <num_blocks, CUDA_SQRT_BLOCK_SIZE, 0, stream >> > (x, dst, k);
}

void sin_f32_cuda(const float* x, float* dst, const int k, cudaStream_t stream) {
    static constexpr size_t CUDA_SIN_BLOCK_SIZE = 256;
    const int num_blocks = (k + CUDA_SIN_BLOCK_SIZE - 1) / CUDA_SIN_BLOCK_SIZE;
    transform <sinf> << <num_blocks, CUDA_SIN_BLOCK_SIZE, 0, stream >> > (x, dst, k);
}

void cos_f32_cuda(const float* x, float* dst, const int k, cudaStream_t stream) {
    static constexpr size_t CUDA_COS_BLOCK_SIZE = 256;
    const int num_blocks = (k + CUDA_COS_BLOCK_SIZE - 1) / CUDA_COS_BLOCK_SIZE;
    transform <cosf> << <num_blocks, CUDA_COS_BLOCK_SIZE, 0, stream >> > (x, dst, k);
}
