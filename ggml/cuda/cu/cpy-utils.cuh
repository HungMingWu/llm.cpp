...
static __device__ void quantize_block(const float* x, block_q4_0* y) {
    float amax = 0.0f;
    float vmax = 0.0f;
    ...
    y->d = std::bit_cast<uint16_t>(__float2half(d));

    for (int j = 0; j < block_q4_0::block_size / 2; ++j) {
        const float x0 = x[0 + j] * id;
        const float x1 = x[block_q4_0::block_size / 2 + j] * id;
...
static __device__ void quantize_block(const float* x, block_q4_1* y) {
    float vmin = FLT_MAX;
    float vmax = -FLT_MAX;
    ...
    y->dm = std::bit_cast<uint32_t>(dm);

    for (int j = 0; j < block_q4_1::block_size / 2; ++j) {
        const float x0 = (x[0 + j] - vmin) * id;
        const float x1 = (x[block_q4_1::block_size / 2 + j] - vmin) * id;
...
static __device__ void quantize_block(const float* x, block_q5_0* y) {
    float amax = 0.0f;
    float vmax = 0.0f;
    ...
    for (int j = 0; j < block_q5_0::block_size / 2; ++j) {
        const float x0 = x[0 + j] * id;
        const float x1 = x[block_q5_0::block_size / 2 + j] * id;
...
static __device__ void quantize_block(const float* x, block_q5_1* y) {
    float min = x[0];
    float max = x[0];
    ...
    for (int j = 0; j < block_q5_1::block_size / 2; ++j) {
        ...
        const float x1 = (x[block_q5_1::block_size / 2 + j] - min) * id;
...
static __device__ void quantize_block(const float* x, block_q8_0* y) {
...
static __device__ void quantize_block(const float* x, block_iq4_nl* y) {
    float amax = 0.0f;
    float vmax = 0.0f;
    ...
    for (int j = 0; j < block_iq4_nl::block_size / 2; ++j) {
        const float x0 = x[0 + j] * id;
        const float x1 = x[block_iq4_nl::block_size / 2 + j] * id;
        ...
        const float w1 = x[block_iq4_nl::block_size / 2 + j] * x[block_iq4_nl::block_size / 2 + j];
        sumqx += w0 * v0 * x[j] + w1 * v1 * x[block_iq4_nl::block_size / 2 + j];
        sumq2 += w0 * v0 * v0 + w1 * v1 * v1;
    }

    y->d = std::bit_cast<uint16_t>(__float2half(sumq2 > 0 ? sumqx / sumq2 : d));
}