...
void mul_mat_vec_q_cuda(const mat_vec_context* ctx, cudaStream_t stream)
{
    ...
    case GGML_TYPE_Q4_0:
        mul_mat_vec_q_cuda<GGML_TYPE_Q4_0>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q4_1:
        mul_mat_vec_q_cuda<GGML_TYPE_Q4_1>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q5_0:
        mul_mat_vec_q_cuda<GGML_TYPE_Q5_0>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q5_1:
        mul_mat_vec_q_cuda<GGML_TYPE_Q5_1>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q8_0:
        mul_mat_vec_q_cuda<GGML_TYPE_Q8_0>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q2_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q2_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q3_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q3_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q4_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q4_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q5_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q5_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q6_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q6_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ2_XXS:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ2_XXS>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ2_XS:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ2_XS>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ2_S:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ2_S>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ3_XXS:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ3_XXS>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ1_S:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ1_S>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ1_M:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ1_M>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ4_NL:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ4_NL>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ4_XS:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ4_XS>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ3_S:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ3_S>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    default:
        GGML_ABORT("fatal error");
...
using load_tiles_mmq_t = void (*)(const char* __restrict__ x, int* x_tile, const int& kbx0, const int& i_max, const int& stride);
using vec_dot_mmq_t = void (*)(const int* __restrict__ x, const int* __restrict__ y, float* __restrict__ sum, const int& k00);
using mmq_write_back_t = void (*)(const float* __restrict__ sum, float* __restrict__ dst, const int& stride, const int& i_max, const int& j_max);
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * QI8_0 / 2) {
        ...
        x_df[i * MMQ_MMA_TILE_X_K_Q8_0 + kbxd] = __half2float(std::bit_cast<half>(bxi->d));
#else
        x_df[i * (2 * WARP_SIZE / QI8_0) + i / (QI8_0 / 2) + kbxd] = __half2float(std::bit_cast<half>(bxi->d));
#endif // NEW_MMA_AVAILABLE
    }
}
