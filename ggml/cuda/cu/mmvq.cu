...
static void launch_mul_mat_q(ggml_cuda_pool& pool, const mmq_args& args, cudaStream_t stream) {
    ...
    if (!shared_memory_limit_raised[id]) {
        CUDA_CHECK(cudaFuncSetAttribute(mul_mat_q<type, src_t, mmq_x, MMQ_NWARPS, false>, cudaFuncAttributeMaxDynamicSharedMemorySize, nbytes_shared));
        CUDA_CHECK(cudaFuncSetAttribute(mul_mat_q<type, src_t, mmq_x, MMQ_NWARPS, true>, cudaFuncAttributeMaxDynamicSharedMemorySize, nbytes_shared));
