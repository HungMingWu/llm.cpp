...
template <ggml_type type, typename src_t, int ncols_y>
...
#pragma unroll
            for (int i = 0; i < rows_per_cuda_block; ++i) {
                tmp[j][i] += vec_dot_q(static_cast<const src_t*>(vx) + (row0 + i) * blocks_per_row_x + kbx, &y[j * blocks_per_col_y + kby], kqs);
            }
        }
    }
...
static void mul_mat_vec_q_cuda(
    ...
    switch (ncols_y) {
    case 1:
        mul_mat_vec_q<type, src_t, 1> << <block_nums, block_dims, 0, stream >> > (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
        break;
    case 2:
        mul_mat_vec_q<type, src_t, 2> << <block_nums, block_dims, 0, stream >> > (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
        break;
    case 3:
        mul_mat_vec_q<type, src_t, 3> << <block_nums, block_dims, 0, stream >> > (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
        break;
    case 4:
        mul_mat_vec_q<type, src_t, 4> << <block_nums, block_dims, 0, stream >> > (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
        break;
    case 5:
        mul_mat_vec_q<type, src_t, 5> << <block_nums, block_dims, 0, stream >> > (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
        break;
    case 6:
        mul_mat_vec_q<type, src_t, 6> << <block_nums, block_dims, 0, stream >> > (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
        break;
    case 7:
        mul_mat_vec_q<type, src_t, 7> << <block_nums, block_dims, 0, stream >> > (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
        break;
    case 8:
        mul_mat_vec_q<type, src_t, 8> << <block_nums, block_dims, 0, stream >> > (vx, vy, dst, ncols_x, nrows_x, nrows_y, nrows_dst);
        break;
    default:
        GGML_ABORT("fatal error");
        break;
...
void mul_mat_vec_q_cuda(const mat_vec_context* ctx, cudaStream_t stream)
{
    ...
    case GGML_TYPE_Q4_0:
        mul_mat_vec_q_cuda<GGML_TYPE_Q4_0, block_q4_0>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q4_1:
        mul_mat_vec_q_cuda<GGML_TYPE_Q4_1, block_q4_1>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q5_0:
        mul_mat_vec_q_cuda<GGML_TYPE_Q5_0, block_q5_0>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q5_1:
        mul_mat_vec_q_cuda<GGML_TYPE_Q5_1, block_q5_1>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q8_0:
        mul_mat_vec_q_cuda<GGML_TYPE_Q8_0, block_q8_0>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q2_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q2_K, block_q2_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q3_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q3_K, block_q3_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q4_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q4_K, block_q4_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q5_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q5_K, block_q5_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_Q6_K:
        mul_mat_vec_q_cuda<GGML_TYPE_Q6_K, block_q6_K>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ2_XXS:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ2_XXS, block_iq2_xxs>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ2_XS:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ2_XS, block_iq2_xs>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ2_S:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ2_S, block_iq2_s>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ3_XXS:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ3_XXS, block_iq3_xxs>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ1_S:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ1_S, block_iq1_s>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ1_M:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ1_M, block_iq1_m>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ4_NL:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ4_NL, block_iq4_nl>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ4_XS:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ4_XS, block_iq4_xs>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    case GGML_TYPE_IQ3_S:
        mul_mat_vec_q_cuda<GGML_TYPE_IQ3_S, block_iq3_s>(ctx->vx, ctx->vy, ctx->dst,
            ctx->ncols_x, ctx->nrows_x, ctx->nrows_y, ctx->ncols_y, ctx->nrows_dst, stream);
        break;
    default:
        GGML_ABORT("fatal error");
...
static __device__ void mul_mat_q_process_tile(
    ...
    for (int kb0 = kb0_start; kb0 < kb0_stop; kb0 += blocks_per_iter) {
        load_tiles<mmq_y, nwarps, need_check>((const src_t*)(x), tile_x, stride01 * it * mmq_y + kb0, tile_x_max_i, stride01);
...
#pragma unroll
    ...
    auto [write_dst, stride, i_max, j_max] = [=]() -> std::tuple<float*, int, int, int> {
        if constexpr (fixup) {
            ...
            return { dst + jt * mmq_x * ne0 + it * mmq_y, ne0, tile_x_max_i, tile_y_max_j };
        }
    }();

    if constexpr (new_mma_available_v) {
        mmq_write_back_mma<mmq_x, mmq_y, nwarps, need_check>(sum, write_dst, stride, i_max, j_max);
    }
    else {
        mmq_write_back_dp4a<mmq_x, mmq_y, nwarps, need_check>(sum, write_dst, stride, i_max, j_max);
    }
}
...
static void launch_mul_mat_q(const mat_q_context* ctx, const mmq_args& args, cudaStream_t stream) {
    ...
    if (!shmem_limit_raised[id]) {
        CUDA_CHECK(cudaFuncSetAttribute(mul_mat_q<type, src_t, mmq_x, MMQ_NWARPS, false>, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem));
        CUDA_CHECK(cudaFuncSetAttribute(mul_mat_q<type, src_t, mmq_x, MMQ_NWARPS, true>, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem));
    ...
    if (!args.use_stream_k) {
        if (args.ne01 % mmq_y == 0) {
            ...
            mul_mat_q<type, src_t, mmq_x, MMQ_NWARPS, need_check> << <block_nums_xy_tiling, block_dims, shmem, stream >> >
    ...
    ggml_cuda_pool_alloc<float> tmp_fixup(*ctx->pool, block_nums_mmq.x * mmq_x * mmq_y);

    if (args.ne01 % mmq_y == 0) {
        constexpr bool need_check = false;
        mul_mat_q<type, src_t, mmq_x, MMQ_NWARPS, need_check> << <block_nums_mmq, block_dims, shmem, stream >> >
            (args.x, args.y, args.dst, tmp_fixup.ptr, args.ne00, args.ne01, args.stride01, args.ne10, args.ne11, args.stride11, args.ne0);
        mul_mat_q_stream_k_fixup<type, src_t, mmq_x, MMQ_NWARPS, need_check> << <block_nums_xy_tiling, block_dims, 0, stream >> >
...
template <ggml_type type, typename src_t>
void mul_mat_q_case(const mat_q_context* ctx, const mmq_args& args, cudaStream_t stream) {
    const int id = ggml_cuda_get_device();
    const int nsm = ggml_cuda_info().devices[id].nsm;
    const int cc = ggml_cuda_info().devices[id].cc;
    ...
    switch (mmq_x_best) {
    case   8:
        launch_mul_mat_q<type, src_t, 8>(ctx, args, stream);
        break;
    case  16:
        launch_mul_mat_q<type, src_t, 16>(ctx, args, stream);
        break;
    case  24:
        launch_mul_mat_q<type, src_t, 24>(ctx, args, stream);
        break;
    case  32:
        launch_mul_mat_q<type, src_t, 32>(ctx, args, stream);
        break;
    case  40:
        launch_mul_mat_q<type, src_t, 40>(ctx, args, stream);
        break;
    case  48:
        launch_mul_mat_q<type, src_t, 48>(ctx, args, stream);
        break;
    case  56:
        launch_mul_mat_q<type, src_t, 56>(ctx, args, stream);
        break;
    case  64:
        launch_mul_mat_q<type, src_t, 64>(ctx, args, stream);
        break;
    case  72:
        launch_mul_mat_q<type, src_t, 72>(ctx, args, stream);
        break;
    case  80:
        launch_mul_mat_q<type, src_t, 80>(ctx, args, stream);
        break;
    case  88:
        launch_mul_mat_q<type, src_t, 88>(ctx, args, stream);
        break;
    case  96:
        launch_mul_mat_q<type, src_t, 96>(ctx, args, stream);
        break;
    case 104:
        launch_mul_mat_q<type, src_t, 104>(ctx, args, stream);
        break;
    case 112:
        launch_mul_mat_q<type, src_t, 112>(ctx, args, stream);
        break;
    case 120:
        launch_mul_mat_q<type, src_t, 120>(ctx, args, stream);
        break;
    case 128:
        launch_mul_mat_q<type, src_t, 128>(ctx, args, stream);
        break;
    default:
...
{
    const int cc = ggml_cuda_info().devices[ctx->id].cc;
    ...
    const bool use_stream_k = ggml_cuda_highest_compiled_arch(cc) >= GGML_CUDA_CC_VOLTA &&
        cc < GGML_CUDA_CC_OFFSET_AMD && ctx->src1_ncols == ctx->ne11;
    const mmq_args args = { ctx->src0_dd_i, ctx->src1_ddq_i, ctx->dst_dd_i,
        ctx->ne00, ctx->row_diff, ctx->stride00, ctx->src1_padded_row_size,
        ctx->src1_ncols, ctx->ne11, ctx->nrows_dst, use_stream_k };

    switch (ctx->src0_type) {
    case GGML_TYPE_Q4_0:
        mul_mat_q_case<GGML_TYPE_Q4_0, block_q4_0>(ctx, args, stream);
        break;
    case GGML_TYPE_Q4_1:
        mul_mat_q_case<GGML_TYPE_Q4_1, block_q4_1>(ctx, args, stream);
        break;
    case GGML_TYPE_Q5_0:
        mul_mat_q_case<GGML_TYPE_Q5_0, block_q5_0>(ctx, args, stream);
        break;
    case GGML_TYPE_Q5_1:
        mul_mat_q_case<GGML_TYPE_Q5_1, block_q5_1>(ctx, args, stream);
        break;
    case GGML_TYPE_Q8_0:
        mul_mat_q_case<GGML_TYPE_Q8_0, block_q8_0>(ctx, args, stream);
        break;
    case GGML_TYPE_Q2_K:
        mul_mat_q_case<GGML_TYPE_Q2_K, block_q2_K>(ctx, args, stream);
        break;
    case GGML_TYPE_Q3_K:
        mul_mat_q_case<GGML_TYPE_Q3_K, block_q3_K>(ctx, args, stream);
        break;
    case GGML_TYPE_Q4_K:
        mul_mat_q_case<GGML_TYPE_Q4_K, block_q4_K>(ctx, args, stream);
        break;
    case GGML_TYPE_Q5_K:
        mul_mat_q_case<GGML_TYPE_Q5_K, block_q5_K>(ctx, args, stream);
        break;
    case GGML_TYPE_Q6_K:
        mul_mat_q_case<GGML_TYPE_Q6_K, block_q6_K>(ctx, args, stream);
        break;
    case GGML_TYPE_IQ2_XXS:
        mul_mat_q_case<GGML_TYPE_IQ2_XXS, block_iq2_xxs>(ctx, args, stream);
        break;
    case GGML_TYPE_IQ2_XS:
        mul_mat_q_case<GGML_TYPE_IQ2_XS, block_iq2_xs>(ctx, args, stream);
        break;
    case GGML_TYPE_IQ2_S:
        mul_mat_q_case<GGML_TYPE_IQ2_S, block_iq2_s>(ctx, args, stream);
        break;
    case GGML_TYPE_IQ3_XXS:
        mul_mat_q_case<GGML_TYPE_IQ3_XXS, block_iq3_xxs>(ctx, args, stream);
        break;
    case GGML_TYPE_IQ3_S:
        mul_mat_q_case<GGML_TYPE_IQ3_S, block_iq3_s>(ctx, args, stream);
        break;
    case GGML_TYPE_IQ1_S:
        mul_mat_q_case<GGML_TYPE_IQ1_S, block_iq1_s>(ctx, args, stream);
        break;
    case GGML_TYPE_IQ4_XS:
        mul_mat_q_case<GGML_TYPE_IQ4_XS, block_iq4_xs>(ctx, args, stream);
        break;
    case GGML_TYPE_IQ4_NL:
        mul_mat_q_case<GGML_TYPE_IQ4_NL, block_iq4_nl>(ctx, args, stream);
        break;
    default:
        GGML_ABORT("fatal error");
        break;
