...
bool gguf_read_emplace_helper(const gguf_reader& gr, std::vector<struct gguf_kv>& kv, const std::string& key, const bool is_array, const size_t n) {
    if (is_array) {
        ...
        try {
            ...
            std::println(stderr, "{}: encountered length_error while reading value for key '{}'", __func__, key);
            return false;
        }
        catch (std::bad_alloc&) {
            std::println(stderr, "{}: encountered bad_alloc error while reading value for key '{}'", __func__, key);
            return false;
        }
...
std::optional<gguf_context> gguf_init_from_file_impl(FILE* file) {
    ...
    // file magic
    {
        ...
        if (!ok) {
            std::println(stderr, "{}: failed to read magic", __func__);
            return std::nullopt;
        }

        for (uint32_t i = 0; i < magic.size(); i++) {
            if (magic[i] != GGUF_MAGIC[i]) {
                std::println(stderr, "{}: invalid magic characters: '{}{}{}{}', expected 'GGUF'", __func__, magic[0], magic[1], magic[2], magic[3]);
                return std::nullopt;
            }
        }
    }
    ...
    if (ok && gr.read(ctx.version)) {
        if (ctx.version == 1) {
            std::println(stderr, "{}: GGUFv1 is no longer supported, please use a more up-to-date version", __func__);
            ok = false;
        }
        if (ctx.version > GGUF_VERSION) {
            std::println(stderr, "{}: this GGUF file is version {} but this software only supports up to version {}",
    ...
    if (ok && gr.read(n_tensors)) {
        ...
        if (n_tensors < 0 || n_tensors > int64_t(SIZE_MAX / sizeof(gguf_tensor_info))) {
            std::println(stderr, "{}: number of tensors is {} but must be in [0, {}]",
    ...
    if (ok && gr.read(n_kv)) {
        ...
        if (n_kv < 0 || n_kv > int64_t(SIZE_MAX / sizeof(gguf_kv))) {
            std::println(stderr, "{}: number of key value pairs is {} but must be in [0, {}]",
    ...
    if (!ok) {
        std::println(stderr, "{}: failed to read header", __func__);
        return std::nullopt;
    }
    ...
    // KV pairs
    {
        for (int64_t i = 0; ok && i < n_kv; ++i) {
            ...
            try {
                ...
                std::println(stderr, "{}: encountered length_error while reading key {}", __func__, i);
                ok = false;
            }
            catch (std::bad_alloc&) {
                std::println(stderr, "{}: encountered bad_alloc error while reading key {}", __func__, i);
                ok = false;
            }
            for (size_t j = 0; ok && j < ctx.kv.size(); ++j) {
                if (key == ctx.kv[j].key) {
                    std::println(stderr, "{}: duplicate key '{}' for tensors {} and {}", __func__, key, j, i);
                    ok = false;
                }
            }
            if (!ok) {
            ...
            default:
            {
                std::println(stderr, "{}: key '{}' has invalid GGUF type {}", __func__, key, static_cast<int>(type));
                ok = false;
            } break;
            }
        }
        ...
        if (!ok) {
            std::println(stderr, "{}: failed to read key-value pairs", __func__);
            return std::nullopt;
        }
        ...
        if (ctx.alignment == 0 || (ctx.alignment & (ctx.alignment - 1)) != 0) {
            std::println(stderr, "{}: alignment {} is not a power of 2", __func__, ctx.alignment);
            return std::nullopt;
        }
    }
    ...
    for (int64_t i = 0; ok && i < n_tensors; ++i) {
        ...
        // tensor name
        {
            ...
            try {
                ...
                std::println(stderr, "{}: encountered length_error while reading tensor name {}", __func__, i);
                ok = false;
            }
            catch (std::bad_alloc&) {
                std::println(stderr, "{} encountered bad_alloc error while reading tensor name {}", __func__, i);
                ok = false;
            }
            if (name.length() >= GGML_MAX_NAME) {
                std::println(stderr, "{}: tensor name {} is too long: {} >= {}", __func__, i, name.length(), GGML_MAX_NAME);
                ok = false;
                break;
            }
            ...
            for (int64_t j = 0; ok && j < i; ++j) {
                if (info.t.name == ctx.info[j].t.name) {
                    std::println(stderr, "{}: duplicate tensor name '{}' for tensors {} and {}", __func__, info.t.name, j, i);
                    ok = false;
                    break;
                }
            }
        ...
        // tensor shape
        {
            ...
            if (n_dims > GGML_MAX_DIMS) {
                std::println(stderr, "{}: tensor '{}' has invalid number of dimensions: {} > {}",
            ...
            for (uint32_t j = 0; ok && j < GGML_MAX_DIMS; ++j) {
                ...
                if (info.t.ne[j] < 0) {
                    std::println(stderr, "{}: tensor '{}' dimension {} has invalid number of elements: {} < 0",
            ...
            if (ok && ((INT64_MAX / info.t.ne[1] <= info.t.ne[0]) ||
                ...
                std::println(stderr, "{}: total number of elements in tensor '{}' with shape "
                    "({}, {}, {}, {}) is >= {}",
        ...
        // tensor type
        {
            ...
            if (info.t.type < 0 || info.t.type >= GGML_TYPE_COUNT) {
                std::println(stderr, "{}: tensor '{}' has invalid ggml type {} ({})",
            ...
            if (blck_size == 0 || info.t.ne[0] % blck_size != 0) {
                std::println(stderr, "{}: tensor '{}' of type {} ({}) has {} elements per row, "
    ...
    if (!ok) {
        std::println(stderr, "{}: failed to read tensor info", __func__);
        return std::nullopt;
    }
    ...
    if (fseek(file, GGML_PAD(ftell(file), ctx.alignment), SEEK_SET) != 0) {
        std::println(stderr, "{}: failed to seek to beginning of data section", __func__);
        return std::nullopt;
    }
    ...
    // compute the total size of the data section, taking into account the alignment
    {
        ...
        for (size_t i = 0; i < ctx.info.size(); ++i) {
            ...
            if (ti.offset != ctx.size) {
                std::println(stderr, "{}: tensor '{}' has offset {}, expected {}",
                    __func__, ti.t.name, ti.offset, ctx.size);
                std::println(stderr, "{}: failed to read tensor data", __func__);
                return std::nullopt;
            }
...
std::optional<gguf_context> gguf_init_from_file(const char* fname)
{
    ...
    if (!file) {
        std::println(stderr, "{}: failed to open GGUF file '{}'", __func__, fname);
        return std::nullopt;
    }
