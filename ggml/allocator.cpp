...
void ggml_gallocr::alloc_graph_impl(ggml_cgraph* graph, const int* node_buffer_ids, const int* leaf_buffer_ids) {
	hash_map.clear();
	// allocate leafs
	// these may be tensors that the application is not using in the graph, but may still want to allocate for other purposes
	for (size_t i = 0; i < graph->leafs.size(); i++) {
		auto& leaf = graph->leafs[i];
		allocate_node(leaf, get_node_buffer_id(leaf_buffer_ids, i));
	}
	...
	for (size_t i = 0; i < graph->nodes.size(); i++) {
		...
		if (node->flags & GGML_TENSOR_FLAG_INPUT) {
			allocate_node(node, get_node_buffer_id(node_buffer_ids, i));
		}
		...
		for (auto& src : node->src) {
			...
			if (src->flags & GGML_TENSOR_FLAG_INPUT) {
				allocate_node(src, get_node_buffer_id(node_buffer_ids, i));
			}
		}
	}
...
bool ggml_gallocr::needs_realloc(ggml_cgraph* graph) {
	...
	for (size_t i = 0; i < graph->nodes.size(); i++) {
		...
		for (size_t j = 0; j < node->src.size(); j++) {
			...
			if (!node_needs_realloc(src, &node_alloc->src[j])) {
#ifndef NDEBUG
...
bool ggml_gallocr::reserve_n(ggml_cgraph* graph, const int* node_buffer_ids, const int* leaf_buffer_ids) {
	// reset allocators
	...
	for (size_t i = 0; i < graph->nodes.size(); i++) {
		...
		for (auto [src, node_alloc] : std::views::zip(node->src, node_alloc->src)) {
			...
			if (src->view_src || src->data) {
				node_alloc.buffer_id = -1;
				node_alloc.offset = SIZE_MAX;
				node_alloc.size_max = 0;
			}
			else {
				const auto& hn = hash_map[src];
				node_alloc.buffer_id = hn.buffer_id;
				node_alloc.offset = hn.offset;
				node_alloc.size_max = bufts[hn.buffer_id]->get_alloc_size(src);
			}
		}
	}
	...
	for (size_t i = 0; i < graph->leafs.size(); i++) {
		...
		if (leaf->view_src || leaf->data) {
			leaf_allocs[i].leaf.buffer_id = -1;
			leaf_allocs[i].leaf.offset = SIZE_MAX;
			leaf_allocs[i].leaf.size_max = 0;
		}
		else {
			leaf_allocs[i].leaf.buffer_id = hn.buffer_id;
			leaf_allocs[i].leaf.offset = hn.offset;
			leaf_allocs[i].leaf.size_max = bufts[hn.buffer_id]->get_alloc_size(leaf);
		}
	}
...
bool ggml_gallocr::reserve(ggml_cgraph* graph) {
	return reserve_n(graph, nullptr, nullptr);
}
